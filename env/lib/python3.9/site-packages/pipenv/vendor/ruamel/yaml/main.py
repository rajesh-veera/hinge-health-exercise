...
from pipenv.vendor.ruamel.yaml.loader import (
    BaseLoader,
    SafeLoader,
    Loader,
    RoundTripLoader,
)  # NOQA
from pipenv.vendor.ruamel.yaml.dumper import (
    BaseDumper,
    SafeDumper,
    Dumper,
    RoundTripDumper,
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, "add_implicit_resolver"):
            ...
            Loader,
            (BaseLoader, SafeLoader, ruamel.loader.Loader, RoundTripLoader),
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, "add_implicit_resolver"):
            ...
            Dumper,
            (BaseDumper, SafeDumper, ruamel.dumper.Dumper, RoundTripDumper),
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
