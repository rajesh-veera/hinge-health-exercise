    ...
    if warning_printer.should_warn():
        ...
        if conflicts:
            warning_printer.print_multi_line(
                "Possibly conflicting dependencies found",
        ...
        if cycles:
            warning_printer.print_multi_line(
                "Cyclic dependencies found", lambda: render_cycles_text(cycles)
            )
...
def cyclic_deps(tree: PackageDAG) -> list[list[Package]]:
    ...
    def dfs(
        root: DistPackage, current: Package, visited: set[str], cdeps: list[Package]
    ) -> bool:
