...
def get_installed_distributions(
    ...
    if using_custom_interpreter:
        ...
        result = subprocess.run(
            args, stdout=subprocess.PIPE, check=False, text=True
        )  # noqa: S603
...
def filter_valid_distributions(
    iterable_dists: Iterable[Distribution],
) -> list[Distribution]:
    ...
    for dist in iterable_dists:
        ...
        if warning_printer.should_warn():
            already_seen_dists = first_seen_to_already_seen_dists_dict.setdefault(
    ...
    if warning_printer.should_warn():
        ...
        if first_seen_to_already_seen_dists_dict:
            warning_printer.print_multi_line(
                ...
                lambda: render_duplicated_dist_metadata_text(
