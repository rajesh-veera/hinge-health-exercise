...
def dump_graphviz(  # noqa: C901, PLR0912
    ...
    if output_format not in valid_formats:
        print(
            f"{output_format} is not a supported output format.", file=sys.stderr
        )  # noqa: T201
        print(
            f"Supported formats are: {', '.join(sorted(valid_formats))}",
            file=sys.stderr,
        )  # noqa: T201
        raise SystemExit(1)
    ...
    if is_reverse:
        for dep_rev, parents in tree.items():
            ...
            for parent in parents:
                ...
                edge_label = (
                    parent.req.version_spec if parent.req is not None else None
                ) or "any"
    ...
    if output_format == "dot":
        ...
        # and stable.
        return "".join(
            [next(iter(graph)), *sorted(graph.body), graph._tail]
        )  # noqa: SLF001
