...
from pipenv.patched.pip._internal.utils.egg_link import (
...
def dist_to_frozen_repr(dist: Distribution) -> str:
    ...
    from pipenv.patched.pip._internal.operations.freeze import (
        FrozenRequirement,
    )  # noqa: PLC0415, PLC2701
...
class PipBaseDistributionAdapter:
    ...
    def editable_project_location(self) -> str | None:
        ...
        if direct_url and direct_url.is_local_editable():
            from pipenv.patched.pip._internal.utils.urls import (
                url_to_path,
            )  # noqa: PLC2701, PLC0415
        ...
        if egg_link_path:
            with Path(egg_link_path).open(
                "r", encoding=locale.getpreferredencoding(False)
            ) as f:  # noqa: FBT003
