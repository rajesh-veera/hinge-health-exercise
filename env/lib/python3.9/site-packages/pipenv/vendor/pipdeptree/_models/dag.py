...
class PackageDAG(Mapping[DistPackage, List[ReqPackage]]):
    ...
    def from_pkgs(cls, pkgs: list[Distribution]) -> PackageDAG:
            ...
            while True:
                try:
                    ...
                    if warning_printer.should_warn():
                        dist_name_to_invalid_reqs_dict.setdefault(
                            p.project_name, []
                        ).append(str(err))
                    continue
        ...
        should_print_warning = (
            warning_printer.should_warn() and dist_name_to_invalid_reqs_dict
        )
    ...
    def filter_nodes(
        self, include: list[str] | None, exclude: set[str] | None
        ...
        for node in self._obj:
            ...
            while stack:
                ...
                cldn = [
                    c
                    for c in self._obj[n]
                    if not any(fnmatch(c.key, e) for e in exclude)
                ]
                m[n] = cldn
                seen.add(n.key)
                for c in cldn:
        ...
        non_existent_includes = [
            i
            for i in include_with_casing_preserved
            if canonicalize_name(i) not in matched_includes
        ]
        if non_existent_includes:
            raise ValueError(
                "No packages matched using the following patterns: "
                + ", ".join(non_existent_includes)
            )
