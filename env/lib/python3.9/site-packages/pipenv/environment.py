...
class Environment:
    def __init__(
        ...
        self.base_working_set = (
            base_working_set if base_working_set else BASE_WORKING_SET
        )
        ...
        with self.activated() as active:
            if active:
                ...
                python_version = Version(
                    version_str
                )  # Create PEP 440 compliant version
    ...
    def build_command(
        ...
        if scripts:
            lines.append(
                "u'scripts': u'{{0}}'.format({})".format(
                    sysconfig_line.format("scripts")
                )
            )
        if py_version:
            lines.append(
    ...
    def reverse_dependencies(self):
        ...
        for req in self.get_package_requirements():
            for d in self.reverse_dependency(req):
                ...
                if rdeps.get(name):
                    if not (
                        rdeps[name].get("required") or rdeps[name].get("installed")
                    ):
    ...
    def is_satisfied(self, req: InstallRequirement):
        ...
        if match is not None:
            ...
            elif match.has_metadata("direct_url.json") or (
                req.link and req.link.is_vcs
            ):
