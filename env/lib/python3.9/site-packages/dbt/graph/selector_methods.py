...
class SelectorMethod(metaclass=abc.ABCMeta):
    ...
    def exposure_nodes(
        self, included_nodes: Set[UniqueId]
    ) -> Iterator[Tuple[UniqueId, Exposure]]:
    ...
    def metric_nodes(
        self, included_nodes: Set[UniqueId]
    ) -> Iterator[Tuple[UniqueId, Metric]]:
...
class QualifiedNameSelectorMethod(SelectorMethod):
    def node_is_match(
        self, qualified_name: str, fqn: List[str], is_versioned: bool
    ) -> bool:
...
class TagSelectorMethod(SelectorMethod):
    def search(
        ...
        for unique_id, node in self.all_nodes(included_nodes):
            if hasattr(node, "tags") and any(
                fnmatch(tag, selector) for tag in node.tags
            ):
                yield unique_id
    ...
    def search(
        ...
        for unique_id, node in self.all_nodes(included_nodes):
            ...
            if unique_id in manifest.nodes:
                ...
                previous_node = SourceDefinition.from_resource(
                ...
                previous_node = SemanticModel.from_resource(
                    manifest.semantic_models[unique_id]
                )
            elif unique_id in manifest.unit_tests:
                previous_node = UnitTestDefinition.from_resource(
                    manifest.unit_tests[unique_id]
                )
            elif unique_id in manifest.saved_queries:
                previous_node = SavedQuery.from_resource(
                    manifest.saved_queries[unique_id]
                )
        ...
        if selector == "fresher":
            for unique_id in current_state_sources:
                if unique_id not in previous_state_sources:
                    ...
                    current_state_sources[unique_id] > previous_state_sources[unique_id]
                ):
                    matches.add(unique_id)
