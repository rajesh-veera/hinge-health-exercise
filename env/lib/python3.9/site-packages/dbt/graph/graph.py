...
class Graph:
    ...
    def ancestors(
        ...
        return {
            ...
            for _, child in nx.bfs_edges(
                filtered_graph, node, reverse=True, depth_limit=max_depth
            )
        }
    ...
    def descendants(
        ...
        return {
            child
            for _, child in nx.bfs_edges(filtered_graph, node, depth_limit=max_depth)
        }
    ...
    def exclude_edge_type(self, edge_type_to_exclude):
        return nx.subgraph_view(
            self.graph,
            filter_edge=partial(
                self.filter_edges_by_type, edge_type=edge_type_to_exclude
            ),
        )

    def filter_edges_by_type(self, first_node, second_node, edge_type):
        return (
            self.graph.get_edge_data(first_node, second_node).get("edge_type")
            != edge_type
        )
    ...
    def get_subset_graph(self, selected: Iterable[UniqueId]) -> "Graph":
        ...
        for node in include_nodes:
            if node not in new_graph:
                raise ValueError(
                    "Couldn't find model '{}' -- does it exist or is it disabled?".format(
                        node
                    )
                )
