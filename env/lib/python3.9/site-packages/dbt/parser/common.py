...
Testable = TypeVar(
    "Testable", UnparsedNodeUpdate, UnpatchedSourceDefinition, UnparsedModelUpdate
)
...
class VersionedTestBlock(TestBlock, Generic[Versioned]):
    ...
    @classmethod
    def from_yaml_block(
        cls, src: YamlBlock, target: Versioned
    ) -> "VersionedTestBlock[Versioned]":
        return cls(
            file=src.file,
            data=src.data,
            target=target,
...
class ParserRef:
    ...
    def _add(self, column: HasColumnProps) -> None:
        ...
        if isinstance(column, UnparsedColumn):
            quote = column.quote
            granularity = (
                TimeGranularity(column.granularity) if column.granularity else None
            )
        ...
        self.column_info[column.name] = ColumnInfo(
            ...
            constraints=[
                ColumnLevelConstraint.from_dict(c) for c in column.constraints
            ],
            meta=column.meta,
            tags=tags,
            quote=quote,
    ...
    @classmethod
    def from_versioned_target(
        cls, target: Versioned, version: NodeVersion
    ) -> "ParserRef":
        refs = cls()
