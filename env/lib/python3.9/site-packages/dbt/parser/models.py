        ...
        if node.name == "model":
            ...
            if node.args.args and not node.args.args[0].arg == "dbt":
                self.dbt_errors.append(
                    "'dbt' not provided for model as the first argument"
                )
...
class PythonParseVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if (
            ...
            and node.func.attr in dbt_function_key_words
        ):
...
class ModelParser(SimpleSQLParser[ModelNode]):
    ...
    def populate(
        self, node: ModelNode, config: ContextConfig, statically_parsed: Dict[str, Any]
    ):
