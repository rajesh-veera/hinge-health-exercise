...
class SchemaParser(SimpleParser[YamlBlock, ModelNode]):
    def __init__(
        ...
        self.generic_test_parser = SchemaGenericTestParser(
            project, manifest, root_project
        )
    ...
    def parse_file(self, block: FileBlock, dct: Optional[Dict] = None) -> None:
        ...
        if dct:
            ...
            if "models" in dct:
                ...
                model_parse_result = ModelPatchParser(
                    self, yaml_block, "models"
                ).parse()
            ...
            # PatchParser.parse()
            if "seeds" in dct:
                seed_parse_result = TestablePatchParser(
                    self, yaml_block, "seeds"
                ).parse()
            ...
            if "snapshots" in dct:
                ...
                snapshot_parse_result = TestablePatchParser(
                    self, yaml_block, "snapshots"
                ).parse()
    ...
    def _add_yaml_snapshot_nodes_to_manifest(
        ...
        for snapshot in snapshots:
            if "relation" in snapshot:
                ...
                snapshot_node.raw_code = (
                    "select * from {{ " + snapshot["relation"] + " }}"
                )
...
NodeTarget = TypeVar(
    "NodeTarget", UnparsedNodeUpdate, UnparsedAnalysisUpdate, UnparsedModelUpdate
)
...
class YamlReader(metaclass=ABCMeta):
    def __init__(self, schema_parser: SchemaParser, yaml: YamlBlock, key: str) -> None:
        ...
        self.renderer: SchemaYamlRenderer = SchemaYamlRenderer(
            self.render_ctx, self.key
        )
        ...
        for entry in data:
            ...
            if "versions" in entry:
                for version in entry["versions"]:
                    if "v" in version:
                        unrendered_version_configs[version["v"]] = version.get(
                            "config", {}
                        )
            ...
            if unrendered_config:
                schema_file.add_unrendered_config(
                    unrendered_config, self.key, entry["name"]
                )
            ...
            if unrendered_database:
                schema_file.add_unrendered_database(
                    self.key, entry["name"], unrendered_database
                )
            if unrendered_schema:
                schema_file.add_unrendered_schema(
                    self.key, entry["name"], unrendered_schema
                )

            if self.schema_yaml_vars.env_vars:
                self.schema_parser.manifest.env_vars.update(
...
class SourceParser(YamlReader):
        ...
        for data in self.get_key_dicts():
            ...
            if is_override:
                ...
                if key in self.manifest.source_patches:
                    raise DuplicateSourcePatchNameError(
    ...
    def add_source_definitions(self, source: UnparsedSourceDefinition) -> None:
        ...
        for table in source.tables:
            unique_id = ".".join(
                [NodeType.Source, package_name, source.name, table.name]
            )
...
class NodePatchParser(PatchParser[NodeTarget, ParsedNodePatch], Generic[NodeTarget]):
    def parse_patch(self, block: TargetBlock[NodeTarget], refs: ParserRef) -> None:
        ...
        if patch.yaml_key in ["models", "seeds", "snapshots"]:
            ...
            unique_id = self.manifest.analysis_lookup.get_unique_id(
                patch.name, None, None
            )
        else:
...
class ModelPatchParser(NodePatchParser[UnparsedModelUpdate]):
    ...
    def parse_patch(
        self, block: TargetBlock[UnparsedModelUpdate], refs: ParserRef
    ) -> None:
        target = block.target
        ...
        if not versions:
            ...
            for unparsed_version in versions:
                versioned_model_name = (
                    unparsed_version.defined_in
                    or f"{block.name}_{unparsed_version.formatted_v}"
                )
                ...
                if (
                    versioned_model_unique_id is None
                    and unparsed_version.v == latest_version
                ):
                    versioned_model_unique_id = self.manifest.ref_lookup.get_unique_id(
                ...
                versioned_model_node.unique_id = f"model.{target.package_name}.{target.name}.{unparsed_version.formatted_v}"
                ...
                self.patch_constraints(
                    versioned_model_node, versioned_model_patch.constraints
                )
    ...
    def patch_constraints(
        self, node: ModelNode, constraints: List[Dict[str, Any]]
    ) -> None:
    ...
    def patch_time_spine(
        self, node: ModelNode, time_spine: Optional[TimeSpine]
    ) -> None:
    ...
    def _validate_constraint_prerequisites(self, model_node: ModelNode) -> None:
        ...
        if (
            any(warn_unsupported)
            and not model_node.materialization_enforces_constraints
        ):
            warn_or_error(
                UnsupportedConstraintMaterialization(
        ...
        if str(model_node.language) != "sql":
            errors.append(
                f"Language Error: Expected 'sql' but found '{model_node.language}'"
            )
...
class SingularTestPatchParser(
    PatchParser[UnparsedSingularTestUpdate, ParsedSingularTestPatch]
):
    ...
    def parse_patch(
        self, block: TargetBlock[UnparsedSingularTestUpdate], refs: ParserRef
    ) -> None:
