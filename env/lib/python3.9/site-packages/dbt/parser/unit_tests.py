...
class UnitTestManifestLoader:
    ...
    def _build_fixture_raw_code(
        self, rows, column_name_to_data_types, fixture_format
    ) -> str:
        ...
        if fixture_format == UnitTestFormat.SQL:
            ...
            return (
                "{{{{ get_fixture_sql({rows}, {column_name_to_data_types}) }}}}"
            ).format(rows=rows, column_name_to_data_types=column_name_to_data_types)

    def _get_original_input_node(
        self, input: str, tested_node: ModelNode, test_case_name: str
    ):
        """
...
class UnitTestParser(YamlReader):
    ...
    def parse(self) -> ParseResult:
        for data in self.get_key_dicts():
            ...
            unit_test_config = self._build_unit_test_config(
                unit_test_fqn, unit_test.config
            )
            ...
            if tested_model_node:
                unit_test_definition.depends_on.nodes.append(
                    tested_model_node.unique_id
                )
    ...
    def _validate_and_normalize_rows(
        self, ut_fixture, unit_test_definition, fixture_type
    ) -> None:
        if ut_fixture.format == UnitTestFormat.Dict:
            ...
            if not (
                isinstance(ut_fixture.rows, str) or isinstance(ut_fixture.fixture, str)
            ):
                raise ParsingError(
            ...
            if ut_fixture.fixture:
                csv_rows = self.get_fixture_file_rows(
                    ut_fixture.fixture,
                    self.project.project_name,
                    unit_test_definition.unique_id,
                )
            else:
        ...
        if ut_fixture.rows and (
            ut_fixture.format == UnitTestFormat.Dict
            or ut_fixture.format == UnitTestFormat.CSV
        ):
    ...
    def _load_rows_from_seed(self, ref_str: str) -> List[Dict[str, Any]]:
        ...
        seed_node = self.manifest.ref_lookup.find(
            seed_name, package_name, None, self.manifest
        )
...
def find_tested_model_node(
    ...
    tested_node = manifest.ref_lookup.find(
        model_name, current_project, model_version, manifest
    )
    return tested_node
...
def process_models_for_unit_test(
    ...
    if not unit_test_def.depends_on.nodes:
        tested_node = find_tested_model_node(
            manifest, current_project, unit_test_def.model
        )
        if not tested_node:
            raise ParsingError(
    ...
    target_model_is_incremental = (
        "macro.dbt.is_incremental" in target_model.depends_on.macros
    )
    ...
    if (
        ...
        versioned_models = models_to_versions[target_model.package_name][
            target_model.name
        ]
