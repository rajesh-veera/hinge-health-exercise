...
class MetricParser(YamlReader):
    ...
    def _get_metric_input(
        self, unparsed: Union[UnparsedMetricInput, str]
    ) -> MetricInput:
    ...
    def _get_optional_cumulative_type_params(
        ...
        if unparsed_metric.type.lower() == MetricType.CUMULATIVE.value:
            if not unparsed_type_params.cumulative_type_params:
                unparsed_type_params.cumulative_type_params = (
                    UnparsedCumulativeTypeParams()
                )
    ...
    def parse_metric(
        self, unparsed: UnparsedMetric, generated_from: Optional[str] = None
    ) -> None:
        package_name = self.project.project_name
        ...
        parsed = Metric(
            ...
            time_granularity=(
                TimeGranularity(unparsed.time_granularity)
                if unparsed.time_granularity
                else None
            ),
            filter=parse_where_filter(unparsed.filter),
            meta=unparsed.meta,
...
class SemanticModelParser(YamlReader):
    ...
    def _get_dimensions(
        self, unparsed_dimensions: List[UnparsedDimension]
    ) -> List[Dimension]:
    ...
    def _create_metric(
        ...
        unparsed_metric = UnparsedMetric(
            ...
            type_params=UnparsedMetricTypeParams(
                measure=measure.name, expr=measure.name
            ),
            description=measure.description
            or f"Metric created from measure {measure.name}",
            config={"enabled": enabled},
        )

        parser = MetricParser(self.schema_parser, yaml=self.yaml)
        parser.parse_metric(
            unparsed=unparsed_metric, generated_from=semantic_model_name
        )

    def _generate_semantic_model_config(
        self,
        target: UnparsedSemanticModel,
        fqn: List[str],
        package_name: str,
        rendered: bool,
    ):
...
class SavedQueryParser(YamlReader):
    ...
    def _get_export_config(
        self,
        unparsed_export_config: Dict[str, Any],
        saved_query_config: SavedQueryConfig,
    ) -> ExportConfig:
    ...
    def parse_saved_query(self, unparsed: UnparsedSavedQuery) -> None:
        ...
        for export in parsed.exports:
            ...
            export.config.database = (
                getattr(export, "database", None) or export.config.database
            )
