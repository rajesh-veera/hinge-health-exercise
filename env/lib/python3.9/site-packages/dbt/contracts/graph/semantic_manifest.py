...
class SemanticManifest:
    ...
    def validate(self) -> bool:
        ...
        time_spines_contain_day = any(
            c
            for c in time_spines
            if c.primary_column.time_granularity == TimeGranularity.DAY
        )
        if (
    ...
    def write_json_to_file(self, file_path: str):
        ...
        fire_event(
            ArtifactWritten(
                artifact_type=self.__class__.__name__, artifact_path=file_path
            )
        )
    ...
    def _get_pydantic_semantic_manifest(self) -> PydanticSemanticManifest:
        ...
        for metric in self.manifest.metrics.values():
            pydantic_semantic_manifest.metrics.append(
                PydanticMetric.parse_obj(metric.to_dict())
            )
