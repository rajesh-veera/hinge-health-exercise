...
def package_config_from_data(
    ...
    if unrendered_packages_data:
        ...
        for i in range(0, len(packages_data.get("packages", []))):
            packages_data["packages"][i]["unrendered"] = unrendered_packages_data[
                "packages"
            ][i]
...
class PartialProject(RenderComponents):
    ...
    def check_config_path(
        ...
        if deprecated_path in project_dict:
            if expected_path in project_dict:
                ...
                raise DbtProjectError(
                    msg.format(
                        deprecated_path=deprecated_path, expected_path=expected_path
                    )
                )
    ...
    def create_project(self, rendered: RenderComponents) -> "Project":
        ...
        self.check_config_path(
            rendered.project_dict, "target-path", default_value="target"
        )
        ...
        flag_target_path = (
            str(global_flags.TARGET_PATH) if global_flags.TARGET_PATH else None
        )
        ...
        data_tests = (
            cfg.data_tests if "data_tests" in rendered.project_dict else cfg.tests
        )
    ...
    def from_project_root(
        ...
        *,
        ...
        partial = PartialProject.from_project_root(
            project_root, verify_version=verify_version
        )
