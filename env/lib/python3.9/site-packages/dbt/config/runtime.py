...
class RuntimeConfig(Project, Profile, AdapterRequiredConfig):
    ...
    def from_parts(
        ...
        log_cache_events: bool = getattr(
            args, "log_cache_events", profile.log_cache_events
        )
    ...
    def collect_parts(cls: Type["RuntimeConfig"], args: Any) -> Tuple[Project, Profile]:
        ...
        project = load_project(
            project_root, bool(flags.VERSION_CHECK), profile, cli_vars
        )
        ...
        for resource_type, config_paths in resource_config_paths.items():
            ...
            for config_path in config_paths:
                if not _is_config_used(config_path, fqns):
                    resource_path = ".".join(
                        i for i in ((resource_type,) + config_path)
                    )
