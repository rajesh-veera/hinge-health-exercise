...
class DebugTask(BaseTask):
    ...
    def run(self) -> bool:
        ...
        fire_event(
            DebugCmdOut(msg="Using profiles dir at {}".format(self.profiles_dir))
        )
        fire_event(
            DebugCmdOut(msg="Using profiles.yml file at {}".format(self.profile_path))
        )
        fire_event(
            DebugCmdOut(
        ...
        if self.args.connection:
            ...
            self.test_configuration(
                load_profile_status.log_msg, load_project_status.log_msg
            )
        ...
        if failure_count > 0:
            fire_event(
                DebugCmdResult(
                    msg=red(f"{(pluralize(failure_count, 'check'))} failed:")
                )
            )
