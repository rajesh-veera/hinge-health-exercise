...
class TestRunner(CompileRunner):
    ...
    def print_result_line(self, result):
        ...
        attached_node = (
            ...
            if isinstance(result.node, GenericTestNode)
            else None
        )
    ...
    def build_unit_test_manifest_from_test(
        ...
        loader = UnitTestManifestLoader(
            manifest, self.config, {unit_test_def.unique_id}
        )
        return loader.load()
    ...
    def execute_unit_test(
        ...
        unit_test_manifest = self.build_unit_test_manifest_from_test(
            unit_test_def, manifest
        )
        ...
        # Compile the node
        unit_test_node = self.compiler.compile_node(
            unit_test_node, unit_test_manifest, {}
        )
        assert isinstance(unit_test_node, UnitTestNode)
        ...
        context = generate_runtime_model_context(
            unit_test_node, self.config, unit_test_manifest
        )
        ...
        materialization_macro = unit_test_manifest.find_materialization_macro_by_name(
            self.config.project_name,
            unit_test_node.get_materialization(),
            self.adapter.type(),
        )
    ...
    def build_test_run_result(
        self, test: TestNode, result: TestResultData
    ) -> RunResult:
        ...
        if severity == "ERROR" and result.should_error:
            ...
            if get_flags().WARN_ERROR or get_flags().WARN_ERROR_OPTIONS.includes(
                ...
                status = TestStatus.Fail
                message = (
                    f"Got {num_errors}, configured to fail if {test.config.warn_if}"
                )
            else:
                status = TestStatus.Warn
                message = (
                    f"Got {num_errors}, configured to warn if {test.config.warn_if}"
                )
            failures = result.failures
        else:
    ...
    def _get_daff_diff(
        ...
        expected_daff_table = daff.PythonTableView(
            list_rows_from_table(expected, sort=True)
        )
        actual_daff_table = daff.PythonTableView(
            list_rows_from_table(actual, sort=True)
        )

        flags = daff.CompareFlags()
        flags.ordered = ordered

        alignment = daff.Coopy.compareTables(
            expected_daff_table, actual_daff_table, flags
        ).align()
