...
def print_run_result_error(
    ...
    if result.status == NodeStatus.Fail or (
        ...
        if result.message:
            if is_warning:
                fire_event(
                    RunResultWarningMessage(msg=result.message, node_info=node_info)
                )
            else:
                fire_event(
                    RunResultError(msg=result.message, node_info=node_info, group=group)
                )
        else:
            fire_event(
                RunResultErrorNoMessage(status=result.status, node_info=node_info)
            )

        if result.node.compiled_path is not None:
            fire_event(Formatting(""))
            fire_event(
                SQLCompiledPath(path=result.node.compiled_path, node_info=node_info)
            )
        ...
        if result.node.should_store_failures:
            ...
            fire_event(
                CheckNodeTestFailure(
                    relation_name=result.node.relation_name, node_info=node_info
                )
            )
...
def print_run_end_messages(results, keyboard_interrupt: bool = False) -> None:
    ...
    for error in errors:
        group = (
            group_lookup.get(error.node.unique_id) if hasattr(error, "node") else None
        )
        print_run_result_error(error, is_warning=False, group=group)

    for warning in warnings:
        group = (
            group_lookup.get(warning.node.unique_id)
