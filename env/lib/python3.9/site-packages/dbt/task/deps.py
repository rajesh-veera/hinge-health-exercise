...
def _create_sha1_hash(packages: List[PackageSpec]) -> str:
    ...
    """
    package_strs = [
        json.dumps(package.to_dict(), sort_keys=True) for package in packages
    ]
...
def _create_packages_yml_entry(
    package: str, version: Optional[str], source: str
) -> dict:
...
class DepsTask(BaseTask):
    ...
    def add(self):
        ...
        new_package_entry = _create_packages_yml_entry(
            self.args.add_package["name"],
            self.args.add_package["version"],
            self.args.source,
        )
        ...
        if packages_yml:
            with open(packages_yml_filepath, "w") as pkg_obj:
                pkg_obj.write(
                    yaml.dump(
                        packages_yml, Dumper=dbtPackageDumper, default_flow_style=False
                    )
                )
    ...
    def run(self) -> None:
        ...
        if not system.path_exists(lock_file_path):
            ...
            previous_hash = load_yml_dict(lock_file_path).get(
                PACKAGE_LOCK_HASH_KEY, None
            )
        ...
        packages_lock_dict = load_yml_dict(
            f"{self.project.project_root}/{PACKAGE_LOCK_FILE_NAME}"
        )
