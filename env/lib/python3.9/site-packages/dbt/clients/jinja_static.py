...
def statically_extract_macro_calls(
    source: str,
    ctx: Dict[str, Any],
    db_wrapper: Optional["ParseDatabaseWrapper"] = None,
) -> List[str]:
...
def statically_parse_unrendered_config(string: str) -> Optional[Dict[str, Any]]:
    ...
    if (
        ...
        and _TESTING_MACRO_CACHE
        and string in _TESTING_MACRO_CACHE
    ):
    ...
    config_func_calls = list(
        filter(
            lambda f: hasattr(f, "node")
            and hasattr(f.node, "name")
            and f.node.name == "config",
            func_calls,
        )
    )
