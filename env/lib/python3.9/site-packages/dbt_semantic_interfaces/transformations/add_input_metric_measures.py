...
class AddInputMetricMeasuresRule(
    ...
    def _implements_protocol(
        self,
    ) -> SemanticManifestTransformRule[PydanticSemanticManifest]:  # noqa: D
        return self
    ...
    def _get_measures_for_metric(
        ...
        if matched_metric:
            if (
                matched_metric.type is MetricType.SIMPLE
                or matched_metric.type is MetricType.CUMULATIVE
            ):
                assert (
                ...
                matched_metric.type is MetricType.DERIVED
                or matched_metric.type is MetricType.RATIO
            ):
                for input_metric in matched_metric.input_metrics:
                    measures.update(
                        AddInputMetricMeasuresRule._get_measures_for_metric(
                            semantic_manifest, input_metric.name
                        )
                    )
                ...
                assert (
                    conversion_type_params
                ), "Conversion metric should have conversion_type_params."
    ...
    ) -> PydanticSemanticManifest:  # noqa: D
        for metric in semantic_manifest.metrics:
            measures = AddInputMetricMeasuresRule._get_measures_for_metric(
                semantic_manifest, metric.name
            )
            assert (
                len(metric.type_params.input_measures) == 0
            ), f"{metric} should not have measures predefined"
