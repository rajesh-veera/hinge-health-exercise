...
class SetCumulativeTypeParamsRule(
    ...
    def transform_model(
        ...
        for metric in semantic_manifest.metrics:
            if metric.type == MetricType.CUMULATIVE:
                ...
                if (
                    metric.type_params.window
                    and not metric.type_params.cumulative_type_params.window
                ):
                    metric.type_params.cumulative_type_params.window = (
                        metric.type_params.window
                    )
                if (
                    metric.type_params.grain_to_date
                    and not metric.type_params.cumulative_type_params.grain_to_date
                ):
