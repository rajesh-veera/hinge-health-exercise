    ...
    try:
        ...
        return {
            field.name: FieldDefinition(
                field_type=type_hints[field.name], default_value=field.default
            )
            for field in fields
        }
...
def _is_supported_field_type_in_serializable_dataclass(field_type: Type) -> bool:
    ...
    return (
        ...
        or (
            ...
            and _is_supported_field_type_in_serializable_dataclass(
                _get_type_parameter_for_optional(field_type)
            )
        )
...
class SerializableDataclass(ABC):
    ...
    _concrete_subclass_registry: ClassVar[
        Optional[Set[Type[SerializableDataclass]]]
    ] = None
    ...
    def concrete_subclasses_for_testing(cls) -> Sequence[Type[SerializableDataclass]]:
        ...
        """
        return sorted(
            cls._concrete_subclass_registry or (),
            key=lambda class_type: (class_type.__module__, class_type.__name__),
        )
...
class DataclassSerializer:
    ...
    def _convert_dataclass_instance_to_pydantic_model(
        ...
        if not _is_supported_field_type_in_serializable_dataclass(object_type):
            ...
            if obj is None:
                return None
            optional_field_type_parameter = _get_type_parameter_for_optional(
                object_type
            )
            ...
            tuple_field_type_parameter = (
                _get_type_parameter_for_sequence_like_tuple_type(object_type)
            )
            return tuple(
            ...
            PydanticModel = self._to_pydantic_type_converter.to_pydantic_type(
                object_type
            )
    ...
    def pydantic_serialize(self, obj: SerializableDataclassT) -> str:  # noqa: D
        ...
        assert dataclasses.is_dataclass(
            obj
        ), f"Got object of type: {obj_class.__name__}"
        assert isinstance(
            obj, SerializableDataclass
        ), f"Got object of type: {obj_class.__name__}"
        assert issubclass(
            obj_class, SerializableDataclass
        ), f"Got object type: {obj_class.__name__}"
...
class DataClassDeserializer:
    ...
    def _convert_field_in_pydantic_object_to_actual_object(
        ...
        if not _is_supported_field_type_in_serializable_dataclass(field_type):
            ...
            tuple_type_parameter = _get_type_parameter_for_sequence_like_tuple_type(
                field_type
            )
            return tuple(
            ...
            # Redundant assertion is needed for mypy to pass.
            assert issubclass(
                field_type, SerializableDataclass
            ), f"Got field type: {field_type.__name__}"
            assert isinstance(
                obj, (SerializableDataclass, BaseModel)
            ), f"Got object of type: {obj.__class__.__name__}"
    ...
    def _construct_dataclass_from_dataclass_like_object(
        self,
        dataclass_type: Type[SerializableDataclassT],
        obj: Union[SerializableDataclass, BaseModel],
    ) -> SerializableDataclassT:
        logger.debug(
            f"Constructing dataclass of type {dataclass_type} from {repr(obj)}"
        )
        object_args = {}
    ...
    def pydantic_deserialize(  # noqa: D
        ...
        try:
            ClassAsPydantic = self._to_pydantic_type_converter.to_pydantic_type(
                dataclass_type
            )
            logger.debug(
                f"Serialized object for creation of {ClassAsPydantic} is {serialized_obj}"
            )
...
class DataClassTypeToPydanticTypeConverter:  # noqa: D
    ...
    def to_pydantic_type(
        self, dataclass_type: Type[SerializableDataclass]
    ...
    def _convert_dataclass_type_to_pydantic_type(
        ...
        for field_name, field_definition in field_dict.items():
            ...
            fields_for_pydantic_model[
                field_name
            ] = field_definition.as_pydantic_field_tuple()
            logger.debug(
                f"Adding {field_name} with type {field_definition.annotated_field_type}"
            )
        ...
        logger.debug(
            f"Finished converting {dataclass_type.__name__} to a pydantic class"
        )
    ...
    def _convert_nested_fields(field_definition: FieldDefinition) -> FieldDefinition:
        ...
        """
        if not _is_supported_field_type_in_serializable_dataclass(
            field_definition.annotated_field_type
        ):
            raise RuntimeError(
                f"Unsupported type: {field_definition.annotated_field_type}"
            )
        elif _is_optional_type(field_definition.annotated_field_type):
            optional_field_type_parameter = _get_type_parameter_for_optional(
                field_definition.annotated_field_type
            )
            converted_field_definition = (
                DataClassTypeToPydanticTypeConverter._convert_nested_fields(
            ...
            tuple_field_type_parameter = (
                _get_type_parameter_for_sequence_like_tuple_type(
                    field_definition.annotated_field_type
                )
            )
            converted_field_definition = (
...
class FieldDefinition:
    ...
    def as_pydantic_field_tuple(self) -> Tuple[Type, AnyValueType]:
        ...
        default = (
            self.default_value if self.default_value != dataclasses.MISSING else ...
        )
