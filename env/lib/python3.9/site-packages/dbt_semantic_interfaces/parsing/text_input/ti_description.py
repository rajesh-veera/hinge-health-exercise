    ...
    def __post_init__(self) -> None:  # noqa: D105
        ...
        if item_type is QueryItemType.ENTITY or item_type is QueryItemType.METRIC:
            if self.time_granularity_name is not None:
                raise InvalidQuerySyntax(
                    f"{self.time_granularity_name=} is not supported for {item_type=}"
                )
            if self.date_part_name is not None:
                raise InvalidQuerySyntax(
                    f"{self.date_part_name=} is not supported for {item_type=}"
                )
        elif (
            item_type is QueryItemType.TIME_DIMENSION
            or item_type is QueryItemType.DIMENSION
        ):
            pass
        else:
        ...
        if item_type is QueryItemType.METRIC:
            if len(self.entity_path) > 0:
                raise InvalidQuerySyntax(
                    "The entity path should not be specified for a metric."
                )
            if len(structured_item_name.entity_links) > 0:
                raise InvalidQuerySyntax(
                    "The name of the metric should not have entity links."
                )
        # Check that dimensions / time dimensions have a valid date part.
            ...
            if self.date_part_name is not None:
                ...
                if self.date_part_name.lower() not in set(
                    date_part.value for date_part in DatePart
                ):
                    raise InvalidQuerySyntax(
            ...
            if item_type is QueryItemType.METRIC:
                ...
                if len(self.group_by_for_metric_item) > 0:
                    raise InvalidQuerySyntax(
                        "A group-by should only be specified for metrics."
                    )
            else:
                assert_values_exhausted(item_type)
