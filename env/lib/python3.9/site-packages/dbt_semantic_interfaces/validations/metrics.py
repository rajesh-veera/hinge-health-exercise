...
class CumulativeMetricRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation ensuring cumulative metrics are valid"
    )
    def validate_manifest(
        ...
        for metric in semantic_manifest.metrics or []:
            ...
            if (
                metric.type_params.cumulative_type_params
                and metric.type_params.cumulative_type_params.grain_to_date
            ):
                ...
                try:
                    ...
                    issues.append(
                        ValidationError(
                            context=metric_context,
                            message="".join(
                                traceback.format_exception_only(type(e), value=e)
                            ),
...
class DerivedMetricRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking that the alias set are not unique and distinct"
    )
    ...
    def _validate_input_metrics_exist(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking that input metric time offset params are valid"
    )
                ...
                issues.append(
                    ValidationError(
                        context=MetricContext(
                            file_context=FileContext.from_metadata(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking that the expr field uses the input metrics"
    )
    ...
    def validate_manifest(
        ...
        issues += DerivedMetricRule._validate_input_metrics_exist(
...
class WhereFiltersAreParseable(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    def _validate_time_granularity_names(
        ...
        for _, parameter_set in filter_expression_parameter_sets:
            for (
                time_dim_call_parameter_set
            ) in parameter_set.time_dimension_call_parameter_sets:
                if not time_dim_call_parameter_set.time_granularity_name:
                    continue
                if (
                    time_dim_call_parameter_set.time_granularity_name.lower()
                    not in valid_granularity_names
                ):
                    issues.append(
                        ValidationWarning(
                            context=context,
    ...
    def _validate_metric(
        metric: Metric, custom_granularity_names: List[str]
    ) -> Sequence[ValidationIssue]:  # noqa: D
        issues: List[ValidationIssue] = []
        context = MetricContext(
            file_context=FileContext.from_metadata(metadata=metric.metadata),
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running manifest validation ensuring all metric where filters are parseable"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
...
class ConversionMetricRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking that the params of metric are valid if it is a conversion metric"
    )
    def _validate_type_params(
        metric: Metric, conversion_type_params: ConversionTypeParams
    ) -> List[ValidationIssue]:
        issues: List[ValidationIssue] = []
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checks that the entity exists in the base/conversion semantic model"
    )
    def _validate_entity_exists(
        metric: Metric,
        entity: str,
        base_semantic_model: SemanticModel,
        conversion_semantic_model: SemanticModel,
    ) -> List[ValidationIssue]:
        issues: List[ValidationIssue] = []
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checks that the provided measures are valid for conversion metrics"
    )
    def _validate_measures(
        ...
        def _validate_measure(
            ...
            assert (
                measure
            ), f"Measure '{model_measure.name}' wasn't found in semantic model '{semantic_model.name}'"
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checks that the provided constant properties are valid"
    )
    def _validate_constant_properties(
        ...
        def _elements_in_model(
            references: List[str], semantic_model: SemanticModel
        ) -> None:
        ...
        _elements_in_model(
            references=base_properties, semantic_model=base_semantic_model
        )
        _elements_in_model(
            references=conversion_properties, semantic_model=conversion_semantic_model
        )
        return issues
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running manifest validation ensuring conversion metrics are valid"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
...
class MetricTimeGranularityRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    def _min_queryable_granularity_for_metric(
        ...
        for measure_reference in PydanticMetric.all_input_measures_for_metric(
            ...
            if (
                not min_queryable_granularity
                or defined_time_granularity.to_int()
                > min_queryable_granularity.to_int()
            ):
    ...
    def _validate_metric(
        ...
        if metric.time_granularity:
            min_queryable_granularity = (
                MetricTimeGranularityRule._min_queryable_granularity_for_metric(
                    metric=metric,
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running manifest validation ensuring metric time_granularitys are valid"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:
        ...
        for semantic_model in semantic_manifest.semantic_models:
            dimension_index = {
                DimensionReference(dimension.name): dimension
                for dimension in semantic_model.dimensions
            }
            for measure in semantic_model.measures:
                try:
                    agg_time_dimension_ref = (
                        semantic_model.checked_agg_time_dimension_for_measure(
                            measure.reference
                        )
                    )
