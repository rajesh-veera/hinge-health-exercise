...
class CommonEntitysRule(
    ...
    @staticmethod
    def _map_semantic_model_entities(
        semantic_models: Sequence[SemanticModel],
    ) -> Dict[EntityReference, Set[str]]:
        ...
        for semantic_model in semantic_models or []:
            for entity in semantic_model.entities or []:
                if entity.reference in entities_to_semantic_models:
                    entities_to_semantic_models[entity.reference].add(
                        semantic_model.name
                    )
                else:
                    entities_to_semantic_models[entity.reference] = {
                        semantic_model.name
                    }
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking entity exists on more than one semantic model"
    )
    def _check_entity(
        entity: Entity,
        ...
        if (
            ...
            and len(
                entities_to_semantic_models[entity.reference].difference(
                    {semantic_model.name}
                )
            )
            == 0
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation warning if entities are only one one semantic model"
    )
    def validate_manifest(
        ...
        entities_to_semantic_models = CommonEntitysRule._map_semantic_model_entities(
