...
class MetricLabelsRule(
    ...
    def _check_metric(
        metric: Metric, existing_labels: Dict[str, str]
    ...
    def validate_manifest(
        ...
        for metric in semantic_manifest.metrics:
            issues += MetricLabelsRule._check_metric(
                metric=metric, existing_labels=labels_to_metrics
            )

        return issues


class SemanticModelLabelsRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    @staticmethod
    @validate_safely(
        "checking that a semantic model's dimension labels are unique within itself"
    )
    def _check_semantic_model_dimensions(
    ...
    @staticmethod
    @validate_safely(
        "checking that a semantic model's entity labels are unique within itself"
    )
    ...
    @staticmethod
    @validate_safely(
        "checking that a semantic model's measure labels are unique within itself"
    )
    def _check_semantic_model_measures(
        semantic_model: SemanticModel,
    ) -> Sequence[ValidationIssue]:
        issues: List[ValidationIssue] = []
        label_counts: DefaultDict[str, int] = defaultdict(lambda: 0)
    ...
    def validate_manifest(
        ...
        for semantic_model in semantic_manifest.semantic_models:
            ...
            issues += SemanticModelLabelsRule._check_semantic_model_dimensions(
                semantic_model=semantic_model
            )
            issues += SemanticModelLabelsRule._check_semantic_model_entities(
                semantic_model=semantic_model
            )
...
class EntityLabelsRule(
    ...
    @staticmethod
    @validate_safely(
        "Checking entities of the same name have the same label (or None for the label)"
    )
    def _check_semantic_model_entities(
