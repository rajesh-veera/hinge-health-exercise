...
class SavedQueryRule(
    ...
    * Check that the where filter is valid using the same logic as WhereFiltersAreParsable
    ...
    def _check_group_bys(
        valid_group_by_element_names: Set[str], saved_query: SavedQuery
        ...
        for group_by_item in saved_query.query_params.group_by:
            ...
            element_names_in_group_by = (
                [
                    x.entity_reference.element_name
                    for x in parameter_sets.entity_call_parameter_sets
                ]
                + [
                    x.dimension_reference.element_name
                    for x in parameter_sets.dimension_call_parameter_sets
                ]
                + [
                    x.time_dimension_reference.element_name
                    for x in parameter_sets.time_dimension_call_parameter_sets
                ]
                + [
                    x.metric_reference.element_name
                    for x in parameter_sets.metric_call_parameter_sets
                ]
            )

            if (
                len(element_names_in_group_by) != 1
                or element_names_in_group_by[0] not in valid_group_by_element_names
            ):
                issues.append(
                    ValidationError(
    ...
    def _check_metrics(
        valid_metric_names: Set[str], saved_query: SavedQuery
    ) -> Sequence[ValidationIssue]:
        issues: List[ValidationIssue] = []
    ...
    def _parse_query_item(
        ...
        try:
            item_description = text_processor.get_description(
                query_item_input, valid_method_mapping
            )
            return _ParseQueryItemResult(
                item_description=item_description, validation_issue=None
            )
        except Exception as e:
            return _ParseQueryItemResult(
    ...
    def _check_order_by(saved_query: SavedQuery) -> Sequence[ValidationIssue]:
        ...
        for order_by in saved_query.query_params.order_by:
            ...
            ), "This should have been ensured by the result class."

            # The value of `descending` should be unset as only an order-by item would have it set.
            if (
                item_description.with_descending_unset()
                not in valid_query_item_descriptions
            ):
    ...
    def validate_manifest(
        ...
        valid_group_by_element_names = valid_metric_names.union(
