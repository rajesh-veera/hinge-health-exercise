...
class MetricContext(BaseModel):
    ...
    def context_str(self) -> str:
        """Human-readable stringified representation of the context."""
        return (
            f"with metric `{self.metric.metric_name}` {self.file_context.context_str()}"
        )
...
class ValidationIssue(ABC, BaseModel):
    ...
    def as_readable_str(
        self, verbose: bool = False, prefix: Optional[str] = None
    ) -> str:
    ...
    def as_cli_formatted_str(self, verbose: bool = False) -> str:
        ...
        return self.as_readable_str(
            verbose=verbose,
            prefix=click.style(
                self.level.name, bold=True, fg=ISSUE_COLOR_MAP[self.level]
            ),
        )
...
class ValidationIssueSet:
    ...
    @staticmethod
    def combine(
        validation_issue_sets: Iterable[ValidationIssueSet],
    ) -> ValidationIssueSet:
        ...
        for validation_issue_set in validation_issue_sets:
            ...
            combined_future_error_issues.extend(
                validation_issue_set.future_error_issues
            )
...
class SemanticManifestValidationResults(FrozenBaseModel):
    ...
    @staticmethod
    def from_issues_sequence(
        issues: Sequence[ValidationIssue],
    ) -> SemanticManifestValidationResults:
        """Constructs a SemanticManifestValidationResults class from a list of ValidationIssues."""
        combined_issue_set = ValidationIssueSet.combine(
            tuple(issue.as_issue_set for issue in issues)
        )
    ...
    @classmethod
    def merge(
        cls, results: Sequence[SemanticManifestValidationResults]
    ) -> SemanticManifestValidationResults:
        ...
        future_errors = tuple(
            issue for result in results for issue in result.future_errors
        )
...
class SemanticManifestValidationRule(ABC, Generic[SemanticManifestT]):
    ...
    def validate_manifest(
        cls, semantic_manifest: SemanticManifestT
...
class SemanticModelValidationHelpers:
    ...
    @staticmethod
    def time_dimension_in_model(
        time_dimension_name: str, semantic_model: SemanticModel
    ) -> bool:  # noqa: D
        for dimension in semantic_model.dimensions:
            if (
                dimension.type == DimensionType.TIME
                and dimension.name == time_dimension_name
            ):
                return True
        return False
