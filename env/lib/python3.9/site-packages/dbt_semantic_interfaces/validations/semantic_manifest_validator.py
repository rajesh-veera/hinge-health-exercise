...
class SemanticManifestValidator(Generic[SemanticManifestT]):
    ...
    def __init__(
        self,
        rules: Sequence[
            SemanticManifestValidationRule[SemanticManifestT]
        ] = DEFAULT_RULES,
        max_workers: int = 1,
    ) -> None:
        """Constructor.
    ...
    def _validate_multi_process(  # noqa: D
        ...
        futures = [
            self._executor.submit(
                _validate_manifest_with_one_rule, validation_rule, semantic_manifest
            )
    ...
    def checked_validations(self, semantic_manifest: SemanticManifestT) -> None:
        ...
        semantic_manifest_issues = self.validate_semantic_manifest(
            semantic_manifest_copy
        )
        if semantic_manifest_issues.has_blocking_issues:
            raise SemanticManifestValidationException(
                issues=tuple(semantic_manifest_issues.all_issues)
            )
