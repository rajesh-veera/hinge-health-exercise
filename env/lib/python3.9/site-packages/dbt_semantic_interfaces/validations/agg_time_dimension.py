...
class AggregationTimeDimensionRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking aggregation time dimension for semantic models in the model"
    )
    def validate_manifest(
        ...
        for semantic_model in semantic_manifest.semantic_models:
            issues.extend(
                AggregationTimeDimensionRule._validate_semantic_model(semantic_model)
            )
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking aggregation time dimension for a semantic model"
    )
    def _validate_semantic_model(
        ...
        for measure in semantic_model.measures:
            ...
            agg_time_dimension_reference = (
                semantic_model.checked_agg_time_dimension_for_measure(measure.reference)
            )
            if not SemanticModelValidationHelpers.time_dimension_in_model(
                time_dimension_name=agg_time_dimension_reference.element_name,
