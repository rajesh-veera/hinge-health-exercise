...
class UniqueAndValidNameRule(
    ...
    * Names of semantic models, dimension sets and metric sets in the model are unique / valid.
    ...
    def check_valid_name(
        ...
        if name.lower() in {
            reserved_name.value for reserved_name in MetricFlowReservedKeywords
        }:
            reason = MetricFlowReservedKeywords.get_reserved_reason(
                MetricFlowReservedKeywords(name.lower())
            )
            issues.append(
                ValidationError(
                    context=context,
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking semantic model sub element names are unique"
    )
    def _validate_semantic_model_elements_and_time_spines(
        ...
        for semantic_model in semantic_manifest.semantic_models:
            element_info_tuples: List[
                Tuple[ElementReference, str, ValidationContext]
            ] = []
            for measure in semantic_model.measures:
                custom_granularity_restricted_names_and_types[
                    measure.name
                ] = SemanticModelElementType.MEASURE.value
            ...
            for entity in semantic_model.entities:
                custom_granularity_restricted_names_and_types[
                    entity.name
                ] = SemanticModelElementType.ENTITY.value
                element_info_tuples.append(
                    (
                        entity.reference,
                        "entity",
            ...
            for dimension in semantic_model.dimensions:
                custom_granularity_restricted_names_and_types[
                    dimension.name
                ] = SemanticModelElementType.DIMENSION.value
                element_info_tuples.append(
                    (
                        dimension.reference,
                        "dimension",
            ...
            for name, _, context in element_info_tuples:
                issues += UniqueAndValidNameRule.check_valid_name(
                    name=name.element_name, context=context
                )

        for metric in semantic_manifest.metrics:
            custom_granularity_restricted_names_and_types[
                metric.name
            ] = SemanticManifestNodeType.METRIC.value
        for semantic_model in semantic_manifest.semantic_models:
            custom_granularity_restricted_names_and_types[
                semantic_model.name
        ...
        for time_spine in semantic_manifest.project_configuration.time_spines:
            ...
            for custom_granularity in time_spine.custom_granularities:
                ...
                if (
                    ...
                    in custom_granularity_restricted_names_and_types
                ):
                    issues.append(
                        ValidationError(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking top level elements of a specific type have unique and valid names"
    )
    def _validate_top_level_objects_of_type(
        ...
        for object in objects:
            ...
            issues += UniqueAndValidNameRule.check_valid_name(
                name=object.name, context=context
            )
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking model top level element names are sufficiently unique"
    )
    def _validate_top_level_objects(
        semantic_manifest: SemanticManifest,
    ) -> List[ValidationIssue]:
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation ensuring elements have adequately unique names"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
        issues = []
        issues += UniqueAndValidNameRule._validate_top_level_objects(
            semantic_manifest=semantic_manifest
        )
        issues += (
            UniqueAndValidNameRule._validate_semantic_model_elements_and_time_spines(
                semantic_manifest
            )
        )
...
class PrimaryEntityDimensionPairs(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="validating there are no duplicate dimension primary entity pairs"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
