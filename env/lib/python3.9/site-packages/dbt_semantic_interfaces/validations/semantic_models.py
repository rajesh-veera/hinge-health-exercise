...
class SemanticModelValidityWindowRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking correctness of the time dimension validity parameters in the model"
    )
    def validate_manifest(
        ...
        for semantic_model in semantic_manifest.semantic_models:
            issues.extend(
                SemanticModelValidityWindowRule._validate_semantic_model(
    ...
    def _validate_semantic_model(
        ...
        validity_param_dims = [
            dim for dim in semantic_model.dimensions if dim.validity_params is not None
        ]
        ...
        primary_or_unique_entities = [
            ...
            if entity.type in (EntityType.PRIMARY, EntityType.UNIQUE)
        ]
        if not any(
            [entity.type is EntityType.NATURAL for entity in semantic_model.entities]
        ):
            error = ValidationError(
                context=context,
                message=(
...
class SemanticModelDefaultsRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation ensuring the defaults are valid"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
        ...
        for semantic_model in semantic_manifest.semantic_models:
            issues.extend(
                SemanticModelDefaultsRule._validate_default_agg_time_dimension(
                    semantic_model=semantic_model
                )
            )
        return issues
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking validity of the semantic model's default agg_time_dimension"
    )
    def _validate_default_agg_time_dimension(
        semantic_model: SemanticModel,
    ) -> List[ValidationIssue]:
        issues: List[ValidationIssue] = []

        if (
            semantic_model.defaults is None
            or semantic_model.defaults.agg_time_dimension is None
        ):
            return []
        ...
        if not SemanticModelValidationHelpers.time_dimension_in_model(
            time_dimension_name=default_agg_time_dimension,
            semantic_model=semantic_model,
        ):
            issues.append(
                ValidationError(
