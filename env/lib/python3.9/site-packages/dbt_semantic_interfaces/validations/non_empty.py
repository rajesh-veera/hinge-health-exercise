...
class NonEmptyRule(
    ...
    def _check_model_has_semantic_models(
    ...
    @staticmethod
    @validate_safely(
        "running model validation rule ensuring metrics and semantic models are defined"
    )
    def validate_manifest(
        semantic_manifest: PydanticSemanticManifest,
    ) -> Sequence[ValidationIssue]:  # noqa: D
        issues: List[ValidationIssue] = []
        issues += NonEmptyRule._check_model_has_semantic_models(
            semantic_manifest=semantic_manifest
        )
        issues += NonEmptyRule._check_model_has_metrics(
            semantic_manifest=semantic_manifest
        )
        return issues
