...
class SemanticModelMeasuresUniqueRule(
    ...
    def validate_manifest(
        ...
        measure_references_to_semantic_models: Dict[
            MeasureReference, List
        ] = defaultdict(list)
        for semantic_model in semantic_manifest.semantic_models:
            for measure in semantic_model.measures:
                        ...
                        ValidationError(
                            context=SemanticModelElementContext(
                                file_context=FileContext.from_metadata(
                                    metadata=semantic_model.metadata
                                ),
                ...
                measure_references_to_semantic_models[measure.reference].append(
                    semantic_model.name
                )
...
class MeasureConstraintAliasesRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    def _validate_required_aliases_are_set(
        ...
        for name in input_measures_by_name:
            ...
            constrained_measures_without_aliases = [
                ...
                if measure.filter is not None and measure.alias is None
            ]
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking constrained measures are aliased properly"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
...
class MetricMeasuresRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking all measures referenced by the metric exist"
    )
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation ensuring metric measures exist"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
        issues: List[ValidationIssue] = []
        valid_measure_names = _get_measure_names_from_semantic_manifest(
            semantic_manifest
        )
...
class MeasuresNonAdditiveDimensionRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="ensuring that a measure's non_additive_dimensions is valid"
    )
    def validate_manifest(
        semantic_manifest: SemanticManifestT,
    ) -> Sequence[ValidationIssue]:  # noqa: D
            ...
            for measure in semantic_model.measures:
                ...
                agg_time_dimension = next(
                    (
                        ...
                        if agg_time_dimension_reference.element_name == dim.name
                    ),
                    None,
                )
                ...
                matching_dimension = next(
                    (
                        dim
                        for dim in semantic_model.dimensions
                        if non_additive_dimension.name == dim.name
                    ),
                    None,
                )
                ...
                entities_in_semantic_model = {
                    entity.name for entity in semantic_model.entities
                }
                window_groupings = set(non_additive_dimension.window_groupings)
                intersected_entities = window_groupings.intersection(
                    entities_in_semantic_model
                )
...
class CountAggregationExprRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
...
class PercentileAggregationRule(
    SemanticManifestValidationRule[SemanticManifestT], Generic[SemanticManifestT]
):
        ...
        for semantic_model in semantic_manifest.semantic_models:
            for measure in semantic_model.measures:
                ...
                if measure.agg == AggregationType.PERCENTILE:
                    if (
                        measure.agg_params is None
                        or measure.agg_params.percentile is None
                    ):
                        issues.append(
                            ValidationError(
                                context=context,
                        ...
                        measure.agg_params.percentile <= 0
                        or measure.agg_params.percentile >= 1
                    ):
                        issues.append(
                            ValidationError(
                                context=context,
...
def _get_measure_names_from_semantic_manifest(
