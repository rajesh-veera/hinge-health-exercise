...
class NaturalEntityConfigurationRule(
    ...
    def _validate_semantic_model_natural_entities(
        ...
        if len(natural_entity_names) > 1:
            ...
            issues.append(error)
        if natural_entity_names and not [
            dim for dim in semantic_model.dimensions if dim.validity_params
        ]:
            error = ValidationError(
                context=context,
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="checking that entities marked as EntityType.NATURAL are properly configured"
    )
