...
class ElementConsistencyRule(
    ...
    @staticmethod
    @validate_safely(
        whats_being_done="running model validation ensuring model wide element consistency"
    )
    def validate_manifest(
        semantic_manifest: PydanticSemanticManifest,
    ) -> Sequence[ValidationIssue]:  # noqa: D
        issues = []
        element_name_to_types = ElementConsistencyRule._get_element_name_to_types(
            semantic_manifest=semantic_manifest
        )
        invalid_elements = {
            name: type_mapping
            for name, type_mapping in element_name_to_types.items()
        ...
        for element_name, type_to_context in invalid_elements.items():
            ...
            types_used = [
                SemanticModelElementType(v)
                for v in sorted(k.value for k in type_to_context.keys())
            ]
            for element_type in types_used:
                semantic_model_contexts = type_to_context[element_type]
                semantic_model_names = {
                    ctx.semantic_model.semantic_model_name
                    for ctx in semantic_model_contexts
                }
        ...
        for semantic_model in semantic_manifest.semantic_models:
            ...
            if semantic_model.measures:
                for measure in semantic_model.measures:
                    element_types[measure.name][
                        SemanticModelElementType.MEASURE
                    ].append(semantic_model_context)
            if semantic_model.dimensions:
                for dimension in semantic_model.dimensions:
                    element_types[dimension.name][
                        SemanticModelElementType.DIMENSION
                    ].append(semantic_model_context)
            if semantic_model.entities:
                for entity in semantic_model.entities:
                    element_types[entity.name][SemanticModelElementType.ENTITY].append(
