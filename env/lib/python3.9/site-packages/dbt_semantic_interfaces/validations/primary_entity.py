...
class PrimaryEntityRule(
    ...
    * There should only be one primary entity in the model.
    ...
    @staticmethod
    @validate_safely(
        "Check that a semantic model has properly configured primary entities."
    )
        ...
        if len(entities_with_primary_type) > 0:
            if len(entities_with_primary_type) > 1:
                primary_entity_names = [
                    primary_entity.name for primary_entity in entities_with_primary_type
                ]
                return (
                    ValidationError(
                        message=(
    ...
    @staticmethod
    @validate_safely(
        "Check that semantic models in the manifest have properly configured primary entities."
    )
