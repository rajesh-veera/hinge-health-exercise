...
class PydanticMetricInputMeasure(PydanticCustomInputParser, HashableBaseModel):
    ...
    def _from_yaml_value(
        cls, input: PydanticParseableValueType
...
class PydanticMetric(HashableBaseModel, ModelWithMetadataParsing, ProtocolHint[Metric]):
    ...
    def input_metrics(self) -> Sequence[PydanticMetricInput]:
        ...
        if (
            self.type is MetricType.SIMPLE
            or self.type is MetricType.CUMULATIVE
            or self.type is MetricType.CONVERSION
        ):
            return ()
        elif self.type is MetricType.DERIVED:
            assert (
                self.type_params.metrics is not None
            ), f"{MetricType.DERIVED} should have type_params.metrics set"
            return self.type_params.metrics
        elif self.type is MetricType.RATIO:
            assert (
                self.type_params.numerator is not None
                and self.type_params.denominator is not None
    ...
    def all_input_measures_for_metric(
        ...
        if metric.type is MetricType.SIMPLE or metric.type is MetricType.CUMULATIVE:
            ...
            for input_metric in metric.input_metrics:
                ...
                assert (
                    nested_metric
                ), f"Could not find metric {input_metric.name} in semantic manifest."
                measures.update(
                    PydanticMetric.all_input_measures_for_metric(
                        metric=nested_metric, metric_index=metric_index
                    )
                )
