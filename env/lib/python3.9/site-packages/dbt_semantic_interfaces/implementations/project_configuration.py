...
class PydanticProjectConfiguration(
    HashableBaseModel, ModelWithMetadataParsing, ProtocolHint[ProjectConfiguration]
):
    ...
    time_spine_table_configurations: List[PydanticTimeSpineTableConfiguration] = Field(
    ...
    @classmethod
    def __create_default_dsi_package_version(
        cls, value: Optional[PydanticSemanticVersion]
    ) -> PydanticSemanticVersion:
        ...
        if value is not None and value != UNKNOWN_VERSION_SENTINEL:
            return value
        return PydanticSemanticVersion.create_from_string(
            version("dbt_semantic_interfaces")
        )
