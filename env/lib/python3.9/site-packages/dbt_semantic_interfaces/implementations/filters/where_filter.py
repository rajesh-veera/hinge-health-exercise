...
class PydanticWhereFilter(PydanticCustomInputParser, HashableBaseModel):
    ...
    def _from_yaml_value(
        ...
        if isinstance(input, str):
            ...
            raise ValueError(
                f"Expected input to be of type string, but got type {type(input)} with value: {input}"
            )
...
class PydanticWhereFilterIntersection(HashableBaseModel):
    ...
    def __get_validators__(
        cls,
    ) -> Generator[Callable[[PydanticParseableValueType], Self], None, None]:
    ...
    def _convert_legacy_and_yaml_input(cls, input: PydanticParseableValueType) -> Self:
        ...
        """
        has_legacy_keys = (
            isinstance(input, dict) and cls.__WHERE_SQL_TEMPLATE_FIELD__ in input.keys()
        )
        is_legacy_where_filter = (
            isinstance(input, str)
            or isinstance(input, PydanticWhereFilter)
            or has_legacy_keys
        )
        ...
        for where_filter in self.where_filters:
            try:
                filter_parameter_sets.append(
                    (where_filter.where_sql_template, where_filter.call_parameter_sets)
                )
