...
class StructuredDunderedName:
    ...
    def parse_name(name: str) -> StructuredDunderedName:
        ...
        if associated_granularity:
            ...
            return StructuredDunderedName(
                entity_links=tuple(
                    EntityReference(element_name=entity_name)
                    for entity_name in name_parts[:-2]
                ),
        ...
        # e.g. "messages__ds"
            ...
            return StructuredDunderedName(
                entity_links=tuple(
                    EntityReference(element_name=entity_name)
                    for entity_name in name_parts[:-1]
                ),
    ...
    def dundered_name(self) -> str:
        ...
        items = [
            entity_reference.element_name for entity_reference in self.entity_links
        ] + [self.element_name]
    ...
    def dundered_name_without_entity(self) -> str:
        ...
        return DUNDER.join(
            (self.element_name,)
            + ((self.time_granularity.value,) if self.time_granularity else ())
        )
        ...
        if len(self.entity_links) > 0:
            return DUNDER.join(
                tuple(
                    ...
                    for entity_reference in self.entity_links
                )
            )
