...
def from_json(cls, path, column_names=None, column_types=None, keys=None, **kwargs):
    ...
    if isinstance(path, str) and os.path.isdir(path):
        ...
        for i, filepath in enumerate(filepaths):
            ...
            if keys is not None:
                tables[name] = Table.from_json(
                    filepath, keys[i], column_types=column_types, **kwargs
                )
            else:
                tables[name] = Table.from_json(
                    filepath, column_types=column_types, **kwargs
                )

    else:
        if hasattr(path, "read"):
            js = json.load(
                path, object_pairs_hook=OrderedDict, parse_float=Decimal, **kwargs
            )
        else:
            with open(path) as f:
                js = json.load(
                    f, object_pairs_hook=OrderedDict, parse_float=Decimal, **kwargs
                )
