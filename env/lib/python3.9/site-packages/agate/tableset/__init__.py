...
class TableSet(MappedSequence):
    ...
    def __init__(self, tables, keys, key_name="group", key_type=None, _is_fork=False):
        ...
        if not _is_fork:
            for table in tables:
                if any(
                    not isinstance(a, type(b))
                    for a, b in zip_longest(table.column_types, self._column_types)
                ):
    ...
    def _proxy(self, method_name, *args, **kwargs):
        ...
        return self._fork(tables, self.keys())
...
from agate.tableset.proxy_methods import (
    bins,
    compute,
    denormalize,
    distinct,
