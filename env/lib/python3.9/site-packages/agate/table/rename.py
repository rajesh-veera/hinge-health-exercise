...
def rename(
    ...
    **kwargs
    ...
    if isinstance(column_names, dict):
        column_names = [
            column_names[name] if name in column_names else name
            for name in self._column_names
        ]

    if isinstance(row_names, dict):
        row_names = [
            row_names[name] if name in row_names else name for name in self._row_names
        ]
    ...
    if slug_columns:
        ...
        if column_names is not None:
            if column_names == self._column_names:
                column_names = utils.slugify(
                    column_names, ensure_unique=False, **kwargs
                )
            else:
