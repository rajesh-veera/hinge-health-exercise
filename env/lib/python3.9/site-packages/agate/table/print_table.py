...
def print_table(
    ...
    for column_name in self.column_names[:max_columns]:
        if max_column_width is not None and len(column_name) > max_column_width:
            column_names.append(
                "%s%s" % (column_name[: max_column_width - len_truncation], truncation)
            )
        else:
    ...
    for i, row in enumerate(self._rows):
        ...
        for j, v in enumerate(row):
            if j >= max_columns:
                ...
                v = format_decimal(v, format=number_formatters[j], locale=locale)
            else:
                v = str(v)
    ...
    divider = "{v_line} {columns} {v_line}".format(
        v_line=v_line, columns=" | ".join(h_line * w for w in widths)
    )
