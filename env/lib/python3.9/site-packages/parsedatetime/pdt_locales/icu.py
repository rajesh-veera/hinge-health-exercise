...
def get_icu(locale):
    ...
    result = dict(
        [(key, getattr(base, key)) for key in dir(base) if not key.startswith("_")]
    )
    result["icu"] = None
    ...
    result["Weekdays"] = merge_weekdays(result["Weekdays"], wd[1:] + wd[0:1])
    result["shortWeekdays"] = merge_weekdays(
        result["shortWeekdays"], swd[1:] + swd[0:1]
    )
    result["Months"] = [_sanitize_key(m.lower()) for m in symbols.getMonths()]
    result["shortMonths"] = [
        _sanitize_key(sm.lower()) for sm in symbols.getShortMonths()
    ]
