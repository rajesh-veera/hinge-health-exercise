...
class Instance:
    ...
    def get_owner_dialect_or_config_option(self, option: str, default: Any) -> Any:
        if self.__owner_builder:
            return self.__owner_builder.get_dialect_or_config_option(option, default)
        else:
            return default
...
InstanceSchemaCreator: TypeAlias = Callable[[Instance, Context], Optional[JSONSchema]]
...
def _get_schema_or_none(instance: Instance, ctx: Context) -> Optional[JSONSchema]:
...
def on_dataclass(instance: Instance, ctx: Context) -> Optional[JSONSchema]:
    ...
    if is_dataclass(instance.origin_type):
        ...
        schema = JSONObjectSchema(
            ...
            additionalProperties=jsonschema_config.get("additionalProperties", False),
        )
        ...
        if ctx.all_refs:
            ...
            return JSONSchema(reference=f"{ref_prefix}/{instance.origin_type.__name__}")
        else:
            return schema
...
def on_special_typing_primitive(
    ...
    if is_union(instance.type):
        ...
        if constraints:
            return JSONSchema(
                anyOf=[
                    get_schema(instance.derive(type=arg), ctx) for arg in constraints
                ]
            )
        else:
        ...
        return get_schema(instance.derive(type=instance.type.__supertype__), ctx)
    ...
    #     raise NotImplementedError
        ...
        return get_schema(instance.derive(type=get_args(instance.type)[0]), ctx)
        ...
        for arg_idx, arg in enumerate(args, start=1):
            if not is_unpack(arg):
                ...
                if not unpack_schema:
                    prefix_items.append(get_schema(instance.derive(type=arg), ctx))
            else:
...
def on_named_tuple(instance: Instance, ctx: Context) -> JSONSchema:
    resolved = resolve_type_params(instance.origin_type, get_args(instance.type))[
        instance.origin_type
    ]
    annotations = {
        k: resolved.get(v, v)
        for k, v in getattr(instance.origin_type, "__annotations__", {}).items()
    }
    fields = getattr(instance.type, "_fields", ())
    defaults = getattr(instance.type, "_field_defaults", {})
    as_dict = instance.get_owner_dialect_or_config_option("namedtuple_as_dict", False)
    ...
    for f_name in fields:
        ...
        if f_default is not MISSING:
            if isinstance(f_schema, EmptyJSONSchema):
                f_schema = JSONSchema()
            f_schema.default = _default(f_type, f_default, instance.get_self_config())
        properties[f_name] = f_schema
    if as_dict:
...
def on_typed_dict(instance: Instance, ctx: Context) -> JSONObjectSchema:
    ...
    annotations = {
        k: resolved.get(v, v) for k, v in instance.origin_type.__annotations__.items()
    }
...
def apply_object_constraints(instance: Instance, schema: JSONSchema) -> JSONSchema:
    for annotation in instance.annotations:
...
def on_collection(instance: Instance, ctx: Context) -> Optional[JSONSchema]:
    ...
    if issubclass(instance.origin_type, typing.ByteString):  # type: ignore
        ...
        if is_named_tuple(instance.origin_type):
            return apply_array_constraints(instance, on_named_tuple(instance, ctx))
            ...
            JSONArraySchema(
                items=get_schema(
                    instance=instance.derive(
                        type=(Dict[args[0], args[1]] if args else Dict)  # type: ignore
                    ),
                    ctx=ctx,
                )
            ),
        )
    elif is_generic(instance.type) and issubclass(instance.origin_type, typing.Counter):
        schema = JSONObjectSchema(
    ...
    elif is_generic(instance.type) and issubclass(instance.origin_type, typing.Mapping):
        schema = JSONObjectSchema(
