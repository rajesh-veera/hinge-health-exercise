...
def type_name(
    ...
    if typ is None:
        ...
        if typ in resolved_type_params and resolved_type_params[typ] is not typ:
            return type_name(resolved_type_params[typ], short, resolved_type_params)
        else:
            unpacked_type_arg = get_args(typ)[0]
            if not is_variable_length_tuple(
                unpacked_type_arg
            ) and not is_type_var_tuple(unpacked_type_arg):
                return _get_args_str(unpacked_type_arg, short, resolved_type_params)
...
def is_named_tuple(typ: Type) -> bool:
    try:
        return issubclass(typ, typing.Tuple) and hasattr(typ, "_fields")  # type: ignore
    except TypeError:
        return False
...
def resolve_type_params(
    ...
    if include_bases:
        orig_bases = {
            get_type_origin(orig_base): orig_base for orig_base in get_orig_bases(typ)
        }
...
def get_function_arg_annotation(
    ...
    if isinstance(annotation, str):
        annotation = str_to_forward_ref(annotation, inspect.getmodule(function))
    return annotation
