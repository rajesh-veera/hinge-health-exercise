...
class CodeBuilder:
    def __init__(
        ...
        attrs_registry: typing.Optional[typing.Dict[typing.Any, typing.Any]] = None,
    ):
        self.cls = cls
    ...
    def get_real_type(self, field_name: str, field_type: typing.Type) -> typing.Type:
        cls = self._get_field_class(field_name)
        return substitute_type_params(field_type, self.resolved_type_params[cls])
    ...
    def metadatas(self) -> typing.Dict[str, typing.Mapping[str, typing.Any]]:
        return {name: field.metadata for name, field in self.dataclass_fields.items()}

    @lru_cache(None)
    def get_field_default(self, name: str, call_factory: bool = False) -> typing.Any:
    ...
    def evaluate_forward_ref(
        ...
        globalns = get_forward_ref_referencing_globals(typ, owner, self.globals)
        ...
        try:
            ...
            if filtered_fields:
                if config.forbid_extra_keys:
                    ...
                    self.add_line(f"forbidden_keys = d_keys - {{{allowed_keys_str}}}")
                    with self.indent("if forbidden_keys:"):
                        self.add_line(
                            "raise ExtraKeysError(forbidden_keys,cls) " "from None"
                        )
                ...
                with self.indent("try:"):
                    ...
                    for field_block in field_blocks:
                        ...
                        if field_block.in_kwargs:
                            ...
                            if field_block.fname in kw_only_fields or in_kwargs:
    ...
    def _add_unpack_method_definition(self, method_name: str) -> None:
        kwargs = ""
        default_kwargs = self.get_unpack_method_default_flag_values(pass_decoder=True)
        if default_kwargs:
    ...
    def get_discriminator(
        ...
        if look_in_parents:
            ...
            classes = (self.cls,)
        for cls in classes:
            discriminator = self.get_config(cls, look_in_parents=False).discriminator
            if discriminator:
    ...
    def get_unpack_method_default_flag_values(self, pass_decoder: bool = False) -> str:
        pos_param_names = []
        pos_param_values = []
        kw_param_names = []
        kw_param_values = []
    ...
    def _add_pack_method_lines(self, method_name: str) -> None:
        ...
        try:
            ...
            if pre_serialize:
                if self.is_code_generation_option_enabled(ADD_SERIALIZATION_CONTEXT):
                    pre_serialize_args = "context=context"
                else:
                    pre_serialize_args = ""
                self.add_line(f"self = self.{__PRE_SERIALIZE__}({pre_serialize_args})")
            by_alias_feature = self.is_code_generation_option_enabled(
                TO_DICT_ADD_BY_ALIAS_FLAG
            )
            omit_none_feature = self.is_code_generation_option_enabled(
            ...
            omit_none = self.get_dialect_or_config_option("omit_none", False)
            omit_default = self.get_dialect_or_config_option("omit_default", False)
                    ...
                    if fname in nullable_fields:
                        ...
                        with self.indent("if value is not None:"):
                            self._pack_method_set_value(
                                ...
                                by_alias_feature=by_alias_feature,
                                packed_value=packer,
                                omit_default=(omit_default and default is not None),
                            )
            ...
            if self.encoder is not None:
                if self.encoder_kwargs:
                    ...
                    return_statement = f"return encoder({{}}, {encoder_options})"
                else:
            ...
            if post_serialize:
                ...
                self.add_line(
                    return_statement.format(f"self.{__POST_SERIALIZE__}({kwargs})")
                )
            else:
    ...
    def _add_pack_method_with_dialect_lines(self, method_name: str) -> None:
        packer_args = ", ".join(filter(None, ("self", self.get_pack_method_flags())))
    ...
    def _add_pack_method_definition(self, method_name: str) -> None:
        kwargs = ""
        default_kwargs = self.get_pack_method_default_flag_values(pass_encoder=True)
        if default_kwargs:
            kwargs += f", {default_kwargs}"
    ...
    def get_field_default_literal(self, value: typing.Any) -> str:
        if isinstance(value, enum.IntFlag):
            ...
            isinstance(value, float) and not math.isnan(value) and not math.isinf(value)
        ):
            return repr(value)
    ...
    def build(
        ...
        *,
        ...
        field_type = self.parent.get_type_name_identifier(
            ftype,
            resolved_type_params=self.parent.get_field_resolved_type_params(fname),
        )
        could_be_none = (
            ftype in (typing.Any, type(None), None)
            or is_type_var_any(self.parent.get_real_type(fname, ftype))
            or is_optional(ftype, self.parent.get_field_resolved_type_params(fname))
            or default is None
        )
