...
class AttrsHolder:
    def __new__(cls, name: Optional[str] = None, *args: Any, **kwargs: Any) -> Any:
...
class AbstractMethodBuilder(ABC):
    ...
    def _generate_method_name(self, spec: ValueSpec) -> str:  # pragma: no cover
    ...
    @abstractmethod
    def _add_body(self, spec: ValueSpec, lines: CodeLines) -> None:  # pragma: no cover
        raise NotImplementedError

    def _add_setattr(self, spec: ValueSpec, method_name: str, lines: CodeLines) -> None:
...
class Registry:
    ...
    def get(self, spec: ValueSpec) -> Expression:
        ...
        for packer in self._registry:
            ...
            if expr is not None:
                return expr
        raise UnserializableField(spec.field_ctx.name, spec.type, spec.builder.cls)
...
def ensure_generic_collection_subclass(spec: ValueSpec, *checked_types: Type) -> bool:
    return issubclass(spec.origin_type, checked_types) and ensure_generic_collection(
        spec
    )
