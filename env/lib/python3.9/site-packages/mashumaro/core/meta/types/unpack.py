...
class AbstractUnpackerBuilder(AbstractMethodBuilder, ABC):
    def _generate_method_name(self, spec: ValueSpec) -> str:
        ...
        if spec.field_ctx.name:
            ...
            suffix = ""
        return (
            f"__unpack_{prefix}{spec.builder.cls.__name__}{suffix}" f"__{random_hex()}"
        )
    ...
    def _add_body(self, spec: ValueSpec, lines: CodeLines) -> None:
        for literal_value in get_literal_values(spec.type):
            if isinstance(literal_value, enum.Enum):
                ...
                enum_type_name = spec.builder.get_type_name_identifier(lit_type)
                with lines.indent(
                    f"if value == {enum_type_name}.{literal_value.name}.value:"
                ):
                    lines.append(f"return {enum_type_name}.{literal_value.name}")
...
class DiscriminatedUnionUnpackerBuilder(AbstractUnpackerBuilder):
    ...
    def _add_body(self, spec: ValueSpec, lines: CodeLines) -> None:
        ...
        variant_method_call = self._get_variant_method_call(variant_method_name, spec)
        ...
        if discriminator.field:
            ...
            with lines.indent("except (KeyError, AttributeError):"):
                ...
                with lines.indent(f"for variant in {variants}:"):
                    if discriminator.variant_tagger_fn is not None:
                        self._add_register_variant_tags(lines, variant_tagger_expr)
                    else:
                        with lines.indent("try:"):
                            self._add_register_variant_tags(lines, variant_tagger_expr)
                        with lines.indent("except KeyError:"):
            ...
            with lines.indent(f"for variant in {variants}:"):
                ...
                lines.append("except Exception: pass")
            lines.append(
                f"raise SuitableVariantNotFoundError({variants_type_expr}) " "from None"
            )
...
def _unpack_with_annotated_serialization_strategy(
    ...
    if isinstance(value_type, ForwardRef):
        value_type = spec.builder.evaluate_forward_ref(value_type, spec.origin_type)
    ...
    if field_metadata.get("serialization_strategy") is strategy:
        new_spec.field_ctx.metadata = {
            k: v for k, v in field_metadata.items() if k != "serialization_strategy"
        }
...
def _unpack_annotated_serializable_type(
    ...
    value_type = substitute_type_params(
        value_type,
        resolve_type_params(spec.origin_type, get_args(spec.type))[spec.origin_type],
    )
...
def unpack_generic_serializable_type(spec: ValueSpec) -> Optional[Expression]:
    with suppress(TypeError):
        if issubclass(spec.origin_type, GenericSerializableType):
            type_arg_names = ", ".join(list(map(type_name, get_args(spec.type))))
            field_type = spec.builder.get_type_name_identifier(spec.origin_type)
            return (
                f"{field_type}._deserialize({spec.expression}, " f"[{type_arg_names}])"
            )
    ...
    if is_dataclass(spec.origin_type):
        for annotation in spec.annotations:
            if isinstance(annotation, Discriminator):
                return DiscriminatedUnionUnpackerBuilder(annotation).build(spec)
        ...
        if get_class_that_defines_method(method_name, method_loc) != method_loc and (
    ...
    if is_special_typing_primitive(spec.origin_type):
        if is_union(spec.type):
            ...
            if is_optional(spec.type, resolved_type_params):
                arg = not_none_type_arg(get_args(spec.type), resolved_type_params)
            ...
            raise UnserializableDataError("AnyStr is not supported by mashumaro")
            ...
            return UnpackerRegistry.get(spec.copy(type=spec.type.__supertype__))
            ...
            method_loc = spec.builder.cls if spec.builder.is_nailed else spec.attrs
            if (
                get_class_that_defines_method(method_name, method_loc) != method_loc
            ...
            if spec.builder.is_nailed:
                ...
                self_cls_name = spec.builder.get_type_name_identifier(spec.builder.cls)
            ...
            unpacker = UnpackerRegistry.get(spec.copy(type=get_args(spec.type)[0]))
            return f"*{unpacker}"
        elif is_type_var_tuple(spec.type):
            return UnpackerRegistry.get(spec.copy(type=Tuple[Any, ...]))
        elif isinstance(spec.type, ForwardRef):
            evaluated = spec.builder.evaluate_forward_ref(spec.type, spec.owner)
...
def unpack_named_tuple(spec: ValueSpec) -> Expression:
    ...
    unpackers = []
    as_dict = spec.builder.get_dialect_or_config_option("namedtuple_as_dict", False)
    deserialize_option = get_overridden_deserialization_method(spec)
    if deserialize_option is not None:
    ...
    lines.append(f"setattr({spec.cls_attrs_name}, '{method_name}', {method_name})")
...
def unpack_typed_dict(spec: ValueSpec) -> Expression:
    ...
    annotations = {
        k: resolved.get(v, v) for k, v in spec.origin_type.__annotations__.items()
    }
...
def unpack_collection(spec: ValueSpec) -> Optional[Expression]:
    ...
    def inner_expr(
        ...
        if v_type:
            return UnpackerRegistry.get(spec.copy(type=v_type, expression=v_name))
        else:
    ...
    if issubclass(spec.origin_type, typing.ByteString):  # type: ignore
        ...
        return f"collections.deque([{inner_expr()} " f"for value in {spec.expression}])"
