...
def _pack_with_annotated_serialization_strategy(
    ...
    new_spec = spec.copy(
        type=value_type,
        expression=(f"{spec.self_attrs_name}.{overridden_fn}({spec.expression})"),
    )
...
def _pack_annotated_serializable_type(
    ...
    try:
        ...
        value_type = get_function_return_annotation(spec.origin_type._serialize)
    except (KeyError, ValueError):
...
def pack_union(
    ...
    for type_arg in args:
        packer = PackerRegistry.get(spec.copy(type=type_arg, expression="value"))
    ...
    with lines.indent():
        for packer in packers:
            ...
            if len(packer_arg_type_names) > 1:
                packer_arg_type_check = f"in ({', '.join(packer_arg_type_names)})"
            else:
                packer_arg_type_check = f"is {packer_arg_type_names[0]}"
            if packer == "value":
                with lines.indent(f"if value.__class__ {packer_arg_type_check}:"):
...
def pack_literal(spec: ValueSpec) -> Expression:
    ...
    with lines.indent():
        for literal_value in get_literal_values(spec.type):
            ...
            packer = PackerRegistry.get(spec.copy(type=value_type, expression="value"))
...
def pack_collection(spec: ValueSpec) -> Optional[Expression]:
    ...
    def inner_expr(
        ...
        if v_type:
            return PackerRegistry.get(spec.copy(type=v_type, expression=v_name))
        else:
