...
class Dialect:
    ...
    serialize_by_alias: Union[bool, Literal[Sentinel.MISSING]] = Sentinel.MISSING
    namedtuple_as_dict: Union[bool, Literal[Sentinel.MISSING]] = Sentinel.MISSING
    ...
    def merge(cls, other: Type["Dialect"]) -> Type["Dialect"]:
        ...
        for key, value in other.serialization_strategy.items():
            if isinstance(value, SerializationStrategy):
                serialization_strategy[key] = value
            elif isinstance(serialization_strategy.get(key), SerializationStrategy):
                serialization_strategy[key] = value
            else:
                (
                    serialization_strategy.setdefault(key, {}).update(  # type: ignore
                        value
                    )
                )
