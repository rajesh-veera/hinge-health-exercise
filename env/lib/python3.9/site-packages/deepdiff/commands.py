...
@click.option(
    ...
    required=False,
    default=CUTOFF_DISTANCE_FOR_PAIRS_DEFAULT,
    type=float,
    show_default=True,
)
@click.option(
    ...
    required=False,
    default=CUTOFF_INTERSECTION_FOR_PAIRS_DEFAULT,
    type=float,
    show_default=True,
)
@click.option("--cache-size", required=False, default=0, type=int, show_default=True)
@click.option(
    "--cache-tuning-sample-size", required=False, default=0, type=int, show_default=True
)
@click.option(
    "--cache-purge-level",
    required=False,
...
@click.option(
    "--exclude-paths", required=False, type=str, show_default=False, multiple=True
)
@click.option(
    "--exclude-regex-paths", required=False, type=str, show_default=False, multiple=True
)
...
@click.option(
    "--group-by", required=False, type=str, show_default=False, multiple=False
)
...
@click.option(
    "--log-frequency-in-sec", required=False, default=0, type=int, show_default=True
)
@click.option(
    "--max-passes", required=False, default=10000000, type=int, show_default=True
)
@click.option("--max_diffs", required=False, default=None, type=int, show_default=True)
@click.option(
    "--number-format-notation",
    required=False,
    type=click.Choice(["f", "e"], case_sensitive=True),
    show_default=True,
    default="f",
)
@click.option(
    "--progress-logger",
    required=False,
    type=click.Choice(["info", "error"], case_sensitive=True),
    show_default=True,
    default="info",
)
@click.option("--report-repetition", is_flag=True, show_default=True)
@click.option(
    "--significant-digits", required=False, default=None, type=int, show_default=True
)
@click.option(
    "--truncate-datetime",
    required=False,
    type=click.Choice(["second", "minute", "hour", "day"], case_sensitive=True),
    show_default=True,
    default=None,
)
@click.option(
...
def diff(*args, **kwargs):
    ...
    kwargs["progress_logger"] = (
        logger.info if kwargs["progress_logger"] == "info" else logger.error
    )
    ...
    for name, t_path, t_extension in [
        ("t1", t1_path, t1_extension),
    ...
    if create_patch:
        try:
            ...
            if debug:  # pragma: no cover.
                ...
                sys.exit(
                    f"Error when loading the patch (aka delta): {e}"
                )  # pragma: no cover.
...
def grep(item, path, debug, **kwargs):
    ...
    try:
        ...
        if debug:  # pragma: no cover.
            ...
            sys.exit(
                str(f"Error when running deep search on {path}: {e}")
            )  # pragma: no cover.
