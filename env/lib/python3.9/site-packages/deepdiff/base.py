...
TYPE_STABILIZATION_MSG = (
    "Unable to stabilize the Numpy array {} due to {}. Please set ignore_order=False."
)
    ...
    def get_significant_digits(self, significant_digits, ignore_numeric_type_changes):
        ...
        if significant_digits is None:
            if ignore_numeric_type_changes:
                significant_digits = (
                    DEFAULT_SIGNIFICANT_DIGITS_WHEN_IGNORE_NUMERIC_TYPES
                )
        return significant_digits

    def get_ignore_types_in_groups(
        self,
        ...
        for item_group in ignore_type_in_groups:
            ...
            for item in item_group:
                item = (
                    type(item) if item is None or not isinstance(item, type) else item
                )
