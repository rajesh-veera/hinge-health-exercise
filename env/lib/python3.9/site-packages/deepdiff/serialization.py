...
CSV_HEADER_MAX_CHUNK_SIZE = (
    2048  # The chunk needs to be big enough that covers a couple of rows of data.
)
...
class SerializationMixin:
    def to_json_pickle(self):
        ...
        if jsonpickle:
            ...
            logger.error(
                "jsonpickle library needs to be installed in order to run to_json_pickle"
            )  # pragma: no cover. Json pickle is getting deprecated.
    ...
    def from_json_pickle(cls, value):
        ...
        if jsonpickle:
            ...
            logger.error(
                "jsonpickle library needs to be installed in order to run from_json_pickle"
            )  # pragma: no cover. Json pickle is getting deprecated.
    ...
    def _to_delta_dict(
        self,
        directed=True,
        report_repetition_required=True,
        ...
        **Parameters**
        ...
        if (
            report_repetition_required
            and self.ignore_order
            and not self.report_repetition
        ):
        ...
        if module_dot_class in self.safe_to_import:
            try:
                ...
                raise ModuleNotFoundError(
                    MODULE_NOT_FOUND_MSG.format(module_dot_class)
                ) from None
...
def pickle_load(content=None, file_obj=None, safe_to_import=None):
    ...
    **Examples**
    ...
    if not content and not file_obj:
        raise ValueError(
            "Please either pass the content or the file_obj to pickle_load."
        )
...
def pretty_print_diff(diff):
    ...
    return (
        _get_pretty_form_text(diff.verbose_level)
        .get(diff.report_type, "")
        .format(
            diff_path=diff_path,
            type_t1=type_t1,
            type_t2=type_t2,
...
def load_path_content(path, file_type=None):
    ...
    if file_type == "json":
        ...
        if tomli is None:  # pragma: no cover.
            raise ImportError(
                "On python<=3.10 tomli needs to be installed."
            )  # pragma: no cover.
        ...
        raise UnsupportedFormatErr(
            f"Only json, yaml, toml, csv, tsv and pickle are supported.\n"
...
def save_content_to_path(content, path, file_type=None, keep_backup=True):
    ...
    try:
        _save_content(
            content=content, path=path, file_type=file_type, keep_backup=keep_backup
        )
    except Exception:
...
def _save_content(content, path, file_type, keep_backup=True):
    if file_type == "json":
        ...
        raise UnsupportedFormatErr(
            "Only json, yaml, toml, csv, tsv and pickle are supported.\n"
            f" The {file_type} extension is not known."
        )
    return content
...
def json_convertor_default(default_mapping=None):
    ...
    def _convertor(obj):
        ...
        raise TypeError(
            "We do not know how to convert {} of type {} for json serialization. Please pass the default_mapping parameter with proper mapping of the object to a basic python type.".format(
                obj, type(obj)
            )
        )
