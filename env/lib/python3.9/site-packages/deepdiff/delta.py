...
ELEM_NOT_FOUND_TO_ADD_MSG = (
    "Key or index of {} is not found for {} for setting operation."
)
TYPE_CHANGE_FAIL_MSG = "Unable to do the type change for {} from to type {} due to {}"
VERIFY_BIDIRECTIONAL_MSG = (
    "You have applied the delta to an object that has "
    "different values than the original object the delta was made from."
)
FAIL_TO_REMOVE_ITEM_IGNORE_ORDER_MSG = (
    "Failed to remove index[{}] on {}. It was expected to be {} but got {}"
)
...
DELTA_AT_LEAST_ONE_ARG_NEEDED = (
    "At least one of the diff, delta_path or delta_file arguments need to be passed."
)
INVALID_ACTION_WHEN_CALLING_GET_ELEM = (
    "invalid action of {} when calling _get_elem_and_compare_to_old_value"
)
INVALID_ACTION_WHEN_CALLING_SIMPLE_SET_ELEM = (
    "invalid action of {} when calling _simple_set_elem_value"
)
INVALID_ACTION_WHEN_CALLING_SIMPLE_DELETE_ELEM = (
    "invalid action of {} when calling _simple_set_elem_value"
)
...
class Delta:
    ...
    def __init__(
        ...
        if hasattr(deserializer, "__code__") and "safe_to_import" in set(
            deserializer.__code__.co_varnames
        ):
        ...
        if bidirectional:
            self.always_include_values = (
                True  # We need to include the values in bidirectional deltas
            )
        else:
        ...
        if diff is not None:
            if isinstance(diff, DeepDiff):
                self.diff = diff._to_delta_dict(
                    ...
                    always_include_values=self.always_include_values,
                )
        ...
        self._iterable_compare_func_was_used = self.diff.get(
            "_iterable_compare_func_was_used", iterable_compare_func_was_used
        )
    ...
    def _get_elem_and_compare_to_old_value(
        ...
        try:
            ...
            if isinstance(path_for_err_reporting, (list, tuple)):
                path_for_err_reporting = ".".join(
                    [i[0] for i in path_for_err_reporting]
                )
            if self.bidirectional:
                self._raise_or_log(
                    VERIFICATION_MSG.format(
                        path_for_err_reporting, expected_old_value, current_old_value, e
                    )
                )
            else:
                self._raise_or_log(
                    UNABLE_TO_GET_PATH_MSG.format(path_for_err_reporting)
                )
        return current_old_value

    def _simple_set_elem_value(
        self, obj, path_for_err_reporting, elem=None, value=None, action=None
    ):
        """
            ...
            if action == GET:
                try:
                    ...
                    if elem == len(obj):
                        ...
                        self._raise_or_log(
                            ELEM_NOT_FOUND_TO_ADD_MSG.format(
                                elem, path_for_err_reporting
                            )
                        )
            elif action == GETATTR:
                ...
                raise DeltaError(
                    INVALID_ACTION_WHEN_CALLING_SIMPLE_SET_ELEM.format(action)
                )
        except (KeyError, IndexError, AttributeError, TypeError) as e:
            self._raise_or_log(
                "Failed to set {} due to {}".format(path_for_err_reporting, e)
            )
    ...
    def _coerce_obj(
        ...
        """
        self.post_process_paths_to_convert[elements[:-1]] = {
            "old_type": to_type,
            "new_type": from_type,
        }
    ...
    def _simple_delete_elem(self, obj, path_for_err_reporting, elem=None, action=None):
        ...
        try:
            if action == GET:
                ...
                raise DeltaError(
                    INVALID_ACTION_WHEN_CALLING_SIMPLE_DELETE_ELEM.format(action)
                )
    ...
    def _del_elem(
        ...
        if isinstance(obj, tuple):
            ...
            obj = list(obj)
            obj_is_new = True
        self._simple_delete_elem(
            obj=obj, path_for_err_reporting=path, elem=elem, action=action
        )
    ...
    def _do_iterable_item_added(self):
        ...
        if iterable_item_moved:
            added_dict = {
                v["new_path"]: v["value"] for k, v in iterable_item_moved.items()
            }
    ...
    def _do_post_process(self):
        if self.post_process_paths_to_convert:
            ...
            self._do_values_or_type_changed(
    ...
    def _do_pre_process(self):
        if self._numpy_paths and (
            "iterable_item_added" in self.diff or "iterable_item_removed" in self.diff
        ):
        ...
        try:
            ...
            if len(elements) > 1:
                ...
                if len(elements_subset) != len(elements):
                    ...
                    next_element = None
                parent = self.get_nested_obj(
                    obj=self, elements=elements_subset, next_element=next_element
                )
    ...
    def _do_values_or_type_changed(
        self, changes, is_type_change=False, verify_changes=True
    ):
        for path, value in changes.items():
            ...
            if is_type_change and "new_value" not in value:
                try:
                    ...
                    self._raise_or_log(
                        TYPE_CHANGE_FAIL_MSG.format(
                            obj[elem], value.get("new_type", "unknown"), e
                        )
                    )
                    continue
            else:
    ...
    def _do_item_removed(self, items):
        ...
        try:
            sorted_item = sorted(
                items.items(), key=self._sort_key_for_item_added, reverse=True
            )
        except TypeError:
            sorted_item = sorted(
                items.items(), key=cmp_to_key(self._sort_comparison), reverse=True
            )
        for path, expected_old_value in sorted_item:
            ...
            if (
                look_for_expected_old_value
                and isinstance(obj, list)
                and not self._iterable_compare_func_was_used
            ):
                # It may return None if it doesn't find it
                elem = self._find_closest_iterable_element_for_index(
                    obj, elem, expected_old_value
                )
                if elem is not None:
            ...
            for path, opcodes in _iterable_opcodes.items():
                ...
                for opcode in opcodes:
                    if opcode.tag == "replace":
                        ...
                        transformed.extend(
                            obj[opcode.t1_from_index : opcode.t1_to_index]
                        )
                if is_obj_tuple:
                    obj = tuple(obj)
                    # Making sure that the object is re-instated inside the parent especially if it was immutable
    ...
    def _do_ignore_order_get_old(
        self, obj, remove_indexes_per_path, fixed_indexes_values, path_for_err_reporting
    ):
        """
        ...
        while old_obj_index < max_len:
            ...
            if old_obj_index in remove_indexes_per_path:
                ...
                if current_old_obj == expected_obj_to_delete:
                    ...
                    self._raise_or_log(
                        FAIL_TO_REMOVE_ITEM_IGNORE_ORDER_MSG.format(
                            old_obj_index,
                            path_for_err_reporting,
        ...
        for action, info in self.diff.items():
            ...
            if reverse_action:
                ...
                for path, path_info in info.items():
                    reverse_path = (
                        path_info["new_path"] if path_info.get("new_path") else path
                    )
                    r_diff[action][reverse_path] = {
                        "new_value": path_info["old_value"],
                        "old_value": path_info["new_value"],
                    }
                ...
                for path, path_info in info.items():
                    ...
                    r_diff[action][reverse_path] = {
                        "old_type": path_info["new_type"],
                        "new_type": path_info["old_type"],
                    }
                    if "new_value" in path_info:
                        r_diff[action][reverse_path]["old_value"] = path_info[
                            "new_value"
                        ]
                    if "old_value" in path_info:
                        r_diff[action][reverse_path]["new_value"] = path_info[
                            "old_value"
                        ]
    ...
    @staticmethod
    def _get_flat_row(
        action, info, _parse_path, keys_and_funcs, report_type_changes=True
    ):
    ...
    def _from_flat_dicts(flat_dict_list):
        ...
        for flat_dict in flat_dict_list:
            ...
            if isinstance(path, str):
                ...
                path_str = stringify_path(
                    path, root_element=root_element
                )  # We need the string path
    ...
    def to_flat_dicts(
        self, include_action_in_path=False, report_type_changes=True
    ) -> List[FlatDeltaRow]:
        """
        ...
        **List of actions**
        ...
        return [
            i._asdict()
            for i in self.to_flat_rows(
                include_action_in_path=False, report_type_changes=True
            )
        ]
            ...
            if action in FLATTENING_NEW_ACTION_MAP:
                ...
                for path, values in info.items():
                    ...
                    for value in values:
                        if report_type_changes:
                            row = FlatDeltaRow(
                                path=path, value=value, action=action, type=type(value)
                            )
                        else:
