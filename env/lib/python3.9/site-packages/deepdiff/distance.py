...
class DistanceMixin:
    def _get_rough_distance(self):
        ...
        item = (
            ...
            else self._to_delta_dict(report_repetition_required=False)
        )
    ...
    def __get_item_rough_length(self, item, parent="root"):
        ...
        **parameters**
        ...
        if length is None:
            ...
            length = DeepHash.get_key(
                self.hashes, key=item, default=None, extract_index=1
            )
        return length
            ...
            for removed_hash in hashes_removed:
                try:
                    is_close_distance = self.iterable_compare_func(
                        t2_hashtable[added_hash].item, t1_hashtable[removed_hash].item
                    )
    ...
    def _precalculate_numpy_arrays_distance(
        ...
        if _original_type is None:
            ...
            removed_numpy_compatible_type = (
                get_homogeneous_numpy_compatible_type_of_seq(removed)
            )
            if (
                added_numpy_compatible_type
                and added_numpy_compatible_type == removed_numpy_compatible_type
            ):
...
def _get_item_length(item, parents_ids=frozenset([])):
    ...
    if isinstance(item, Mapping):
        for key, subitem in item.items():
            ...
            if isinstance(key, strings) and (
                key.startswith("_") or key == "deep_distance" or key == "new_path"
            ):
                continue
...
def _get_numbers_distance(num1, num2, max_=1):
    ...
    try:
        ...
        Exception
    ):  # pragma: no cover. I don't think this line will ever run but doesn't hurt to leave it.
...
def _get_timedelta_distance(timedelta1, timedelta2, max_):
    return _get_numbers_distance(
        timedelta1.total_seconds(), timedelta2.total_seconds(), max_
    )
