...
class DeepHash(Base):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.exclude_regex_paths = (
            convert_item_or_items_into_compiled_regexes_else_none(exclude_regex_paths)
        )
        ...
        if self.hashes[UNPROCESSED_KEY]:
            logger.warning(
                "Can not hash the following items: {}.".format(
                    self.hashes[UNPROCESSED_KEY]
                )
            )
        else:
    ...
    def _getitem(hashes, obj, extract_index=0):
        ...
        return (
            result_n_count if extract_index is None else result_n_count[extract_index]
        )
    ...
    def get(self, key, default=None, extract_index=0):
        ...
        """
        return self.get_key(
            self.hashes, key, default=default, extract_index=extract_index
        )
    ...
    def __repr__(self):
        ...
        """
        return short_repr(
            self._get_objects_to_hashes_dict(extract_index=0), max_length=500
        )
    ...
    def values(self):
        return (
            i[0] for i in self.hashes.values()
        )  # Just grab the item and not its count
    ...
    def _prep_obj(self, obj, parent, parents_ids=EMPTY_FROZENSET, is_namedtuple=False):
        ...
        if hasattr(obj, "__slots__"):
            obj_to_dict_strategies.append(
                lambda o: {i: getattr(o, i) for i in o.__slots__}
            )
        else:
            obj_to_dict_strategies.append(
                lambda o: dict(
                    inspect.getmembers(o, lambda m: not inspect.isroutine(m))
                )
            )
    ...
    def _skip_this(self, obj, parent):
        ...
        if self.include_paths and parent != "root":
            ...
            [
                exclude_regex_path.search(parent)
    ...
    def _prep_dict(
        ...
        for key, item in obj.items():
            ...
            key_formatted = (
                "'%s'" % key
                if not print_as_attribute and isinstance(key, strings)
                else key
            )
            ...
            hashed, count = self._hash(
                item, parent=key_in_report, parents_ids=parents_ids_added
            )
    ...
    def _prep_iterable(self, obj, parent, parents_ids=EMPTY_FROZENSET):
        ...
        for i, item in enumerate(obj):
            ...
            parents_ids_added = add_to_frozen_set(parents_ids, item_id)
            hashed, count = self._hash(
                item, parent=new_parent, parents_ids=parents_ids_added
            )
        ...
        if self.ignore_repetition:
            ...
            result = ["{}|{}".format(i, v) for i, v in result.items()]

        result = map(
            str, result
        )  # making sure the result items are string so join command works.
    ...
    def _prep_tuple(self, obj, parent, parents_ids):
        ...
        # It must be a normal tuple
            ...
            result, counts = self._prep_iterable(
                obj=obj, parent=parent, parents_ids=parents_ids
            )
        # We assume it is a namedtuple then
        else:
            result, counts = self._prep_obj(
                obj, parent, parents_ids=parents_ids, is_namedtuple=True
            )
        return result, counts
    ...
    def _hash(self, obj, parent, parents_ids=EMPTY_FROZENSET):
        ...
        if self._skip_this(obj, parent):
            ...
            result, counts = self._prep_tuple(
                obj=obj, parent=parent, parents_ids=parents_ids
            )
