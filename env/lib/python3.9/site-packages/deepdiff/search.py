...
class DeepSearch(dict):
    ...
    **Examples**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.exclude_regex_paths = [
            re.compile(exclude_regex_path) for exclude_regex_path in exclude_regex_paths
        ]
        ...
        if self.use_regexp:
            try:
                ...
                raise TypeError(
                    f"The passed item of {item} is not usable for regex: {e}"
                ) from None
    ...
    def __search_obj(
        self, obj, item, parent, parents_ids=frozenset(), is_namedtuple=False
    ):
        """Search objects"""
        found = False
        if obj == item:
    ...
    def __search_dict(
        self, obj, item, parent, parents_ids=frozenset(), print_as_attribute=False
    ):
        """Search dictionaries"""
        ...
        for item_key in obj_keys:
            ...
            str_item = str(item)
            if (
                (self.match_string and str_item == new_parent_cased)
                or (not self.match_string and str_item in new_parent_cased)
                or (self.use_regexp and item.search(new_parent_cased))
            ):
                self.__report(
                    report_key="matched_paths", key=new_parent, value=obj_child
                )

            self.__search(
                obj_child, item, parent=new_parent, parents_ids=parents_ids_added
            )
    ...
    def __search_str(self, obj, item, parent):
        ...
        elif (self.match_string and item == obj_text) or (
            not self.match_string and item in obj_text
        ):
            is_matched = True
        if is_matched:
            self.__report(report_key="matched_values", key=parent, value=obj)
    ...
    def __search_numbers(self, obj, item, parent):
        if item == obj or (
            not self.strict_checking
            and (item == str(obj) or (self.use_regexp and item.search(str(obj))))
        ):
            self.__report(report_key="matched_values", key=parent, value=obj)
