...
RECORDED_BY_HIGHER_FUNCTION = contextvars.ContextVar(
    "RECORDED_BY_HIGHER_FUNCTION", default=False
)
...
class Diff:
    def __init__(
        self, current_recording_path: str, previous_recording_path: str
    ) -> None:
    ...
    def diff_query_records(self, current: List, previous: List) -> Dict[str, Any]:
        ...
        for i in range(len(current)):
            if current[i].get("result").get("table") is not None:
                current[i]["result"]["table"] = json.loads(
                    current[i]["result"]["table"]
                )
        for i in range(len(previous)):
            if previous[i].get("result").get("table") is not None:
                previous[i]["result"]["table"] = json.loads(
                    previous[i]["result"]["table"]
                )
    ...
    def __init__(
        ...
        if self.previous_recording_path is not None and self.mode in (
            ...
            if self.mode == RecorderMode.REPLAY:
                self._unprocessed_records_by_type = self.load(
                    self.previous_recording_path
                )

    @classmethod
    def register_record_type(cls, rec_type) -> Any:
        cls._record_cls_by_name[rec_type.__name__] = rec_type
        cls._record_name_by_params_name[
            rec_type.params_cls.__name__
        ] = rec_type.__name__
        return rec_type
...
def get_record_mode_from_env() -> Optional[RecorderMode]:
    ...
    # diffing requires a file path, otherwise treat as noop
        ...
        record_mode.lower() == "diff"
        and os.environ.get("DBT_RECORDER_FILE_PATH") is not None
    ):
        return RecorderMode.DIFF
    # replaying requires a file path, otherwise treat as noop
    elif (
        record_mode.lower() == "replay"
        and os.environ.get("DBT_RECORDER_FILE_PATH") is not None
    ):
...
def _get_field(
    field_name: str, t: Type
) -> Tuple[str, Optional[Type], dataclasses.Field]:
