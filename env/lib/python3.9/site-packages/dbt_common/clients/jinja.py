...
from dbt_common.exceptions.macros import (
    MacroReturn,
    UndefinedMacroError,
    CaughtMacroError,
)
...
class MacroFuzzParser(jinja2.parser.Parser):
    ...
    def parse_signature(
        self, node: Union[jinja2.nodes.Macro, jinja2.nodes.CallBlock]
    ) -> None:
...
class MacroFuzzTemplate(jinja2.nativetypes.NativeTemplate):
    ...
    def new_context(
        ...
        return self.environment.context_class(
            self.environment, parent, self.name, self.blocks
        )
...
def _get_blocks_hash(
    text: str, allowed_blocks: Optional[Set[str]], collect_raw_data: bool
) -> int:
